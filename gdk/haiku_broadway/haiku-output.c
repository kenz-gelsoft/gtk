...
static void
haiku_output_send_cmd (BroadwayOutput *output,
...
{
  haiku_output_send_cmd (output, TRUE, BROADWAY_WS_CNX_PONG, NULL, 0);
}

int
haiku_output_flush (BroadwayOutput *output)
{
  if (output->buf->len == 0)
    return TRUE;

  haiku_output_send_cmd (output, TRUE, BROADWAY_WS_BINARY,
...
void
haiku_output_free (BroadwayOutput *output)
{
...
guint32
haiku_output_get_next_serial (BroadwayOutput *output)
{
  return output->serial;
}

void
haiku_output_set_next_serial (BroadwayOutput *output,
				 guint32 serial)
{
  output->serial = serial;
}
...
void
haiku_output_grab_pointer (BroadwayOutput *output,
			      int id,
...
guint32
haiku_output_ungrab_pointer (BroadwayOutput *output)
{
  guint32 serial;
...
void
haiku_output_new_surface(BroadwayOutput *output,
...
void
haiku_output_disconnected (BroadwayOutput *output)
{
  write_header (output, BROADWAY_OP_DISCONNECTED);
}

void
haiku_output_show_surface(BroadwayOutput *output,  int id)
{
...
void
haiku_output_hide_surface(BroadwayOutput *output,  int id)
{
...
void
haiku_output_raise_surface(BroadwayOutput *output,  int id)
{
...
void
haiku_output_lower_surface(BroadwayOutput *output,  int id)
{
...
void
haiku_output_destroy_surface(BroadwayOutput *output,  int id)
{
...
void
haiku_output_set_show_keyboard (BroadwayOutput *output,
                                   gboolean show)
{
...
void
haiku_output_move_resize_surface (BroadwayOutput *output,
				     int             id,
				     gboolean        has_pos,
				     int             x,
				     int             y,
...
void
haiku_output_set_transient_for (BroadwayOutput *output,
				   int             id,
				   int             parent_id)
{
...
void
haiku_output_put_buffer (BroadwayOutput *output,
                            int             id,
