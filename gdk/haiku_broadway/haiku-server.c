...
static void haiku_server_resync_windows (BroadwayServer *server);

static GType haiku_server_get_type (void);

G_DEFINE_TYPE (BroadwayServer, haiku_server, G_TYPE_OBJECT)

static void
haiku_server_init (BroadwayServer *server)
{
  BroadwayWindow *root;
...
static void
haiku_server_class_init (BroadwayServerClass * class)
{
...
static void
haiku_input_free (BroadwayInput *input)
{
...
gboolean
haiku_server_lookahead_event (BroadwayServer  *server,
				 const char         *types)
{
  BroadwayInputMsg *message;
  GList *l;
...
static gboolean
haiku_server_read_all_input_nonblocking (BroadwayInput *input)
{
  GInputStream *in;
  gssize res;
  guint8 buffer[1024];
...
static void
haiku_server_consume_all_input (BroadwayServer *server)
{
...
guint32
haiku_server_get_next_serial (BroadwayServer *server)
{
  if (server->output)
...
void
haiku_server_get_screen_size (BroadwayServer   *server,
				 guint32          *width,
				 guint32          *height)
{
...
void
haiku_server_flush (BroadwayServer *server)
{
  if (server->output &&
...
haiku_server_block_for_input (BroadwayServer *server, char op,
...
guint32
haiku_server_get_last_seen_time (BroadwayServer *server)
{
...
void
haiku_server_query_mouse (BroadwayServer *server,
			     guint32            *toplevel,
			     gint32             *root_x,
			     gint32             *root_y,
			     guint32            *mask)
...
void
haiku_server_destroy_window (BroadwayServer *server,
				gint id)
{
  BroadwayWindow *window;
...
gboolean
haiku_server_window_show (BroadwayServer *server,
			     gint id)
{
  BroadwayWindow *window;
  gboolean sent = FALSE;
...
gboolean
haiku_server_window_hide (BroadwayServer *server,
			     gint id)
{
  BroadwayWindow *window;
  gboolean sent = FALSE;
...
void
haiku_server_window_raise (BroadwayServer *server,
                              gint id)
{
  BroadwayWindow *window;
...
void
haiku_server_set_show_keyboard (BroadwayServer *server,
                                   gboolean show)
{
...
void
haiku_server_window_lower (BroadwayServer *server,
                              gint id)
{
  BroadwayWindow *window;
...
void
haiku_server_window_set_transient_for (BroadwayServer *server,
					  gint id, gint parent)
{
  BroadwayWindow *window;
...
void
haiku_server_window_set_modal_hint (BroadwayServer *server,
...
gboolean
haiku_server_has_client (BroadwayServer *server)
{
  return server->output != NULL;
}

void
haiku_server_window_update (BroadwayServer *server,
			       gint id,
...
gboolean
haiku_server_window_move_resize (BroadwayServer *server,
				    gint id,
				    gboolean with_move,
				    int x,
				    int y,
...
void
haiku_server_focus_window (BroadwayServer *server,
...
guint32
haiku_server_grab_pointer (BroadwayServer *server,
			      gint client_id,
			      gint id,
...
guint32
haiku_server_ungrab_pointer (BroadwayServer *server,
				guint32    time_)
{
  guint32 serial;
...
cairo_surface_t *
haiku_server_open_surface (BroadwayServer *server,
			      guint32 id,
			      char *name,
			      int width,
			      int height)
...
guint32
haiku_server_new_window (BroadwayServer *server,
			    int x,
			    int y,
			    int width,
			    int height,
...
static void
haiku_server_resync_windows (BroadwayServer *server)
{
  GList *l;
