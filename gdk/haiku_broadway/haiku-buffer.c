...
void
haiku_buffer_destroy (BroadwayBuffer *buffer)
{
  g_free (buffer->data);
  g_free (buffer->table);
  g_free (buffer);
...
int
haiku_buffer_get_width (BroadwayBuffer *buffer)
{
  return buffer->width;
}

int
haiku_buffer_get_height (BroadwayBuffer *buffer)
{
...
void
haiku_buffer_encode (BroadwayBuffer *buffer, BroadwayBuffer *prev, GString *dest)
{
  struct entry *entry;
  int i, j, k;
  int x0, x1, y0, y1;
