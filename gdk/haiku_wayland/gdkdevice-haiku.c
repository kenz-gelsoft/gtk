...
typedef struct _GdkHaikuDevicePad GdkHaikuDevicePad;
typedef struct _GdkHaikuDevicePadClass GdkHaikuDevicePadClass;

typedef struct _GdkHaikuTouchData GdkHaikuTouchData;
typedef struct _GdkHaikuPointerFrameData GdkHaikuPointerFrameData;
typedef struct _GdkHaikuPointerData GdkHaikuPointerData;
typedef struct _GdkHaikuTabletData GdkHaikuTabletData;
typedef struct _GdkHaikuTabletToolData GdkHaikuTabletToolData;
typedef struct _GdkHaikuTabletPadGroupData GdkHaikuTabletPadGroupData;
typedef struct _GdkHaikuTabletPadData GdkHaikuTabletPadData;

struct _GdkHaikuTouchData
{
  uint32_t id;
  gdouble x;
  gdouble y;
...
struct _GdkHaikuPointerFrameData
{
  GdkEvent *event;
...
struct _GdkHaikuPointerData {
  ...
  GdkHaikuPointerFrameData frame;
};

struct _GdkHaikuTabletToolData
{
  GdkSeat *seat;
  ...
  GdkDeviceTool *tool;
  GdkHaikuTabletData *current_tablet;
};

struct _GdkHaikuTabletPadGroupData
{
  GdkHaikuTabletPadData *pad;
...
struct _GdkHaikuTabletData
{
  ...
  GdkSeat *seat;
  GdkHaikuPointerData pointer_info;
  GList *events;

  GList *pads;

  GdkHaikuTabletToolData *current_tool;

  gint axis_indices[GDK_AXIS_LAST];
  gdouble *axes;
};

struct _GdkHaikuSeat
{
  ...
  GdkHaikuPointerData pointer_info;
  GdkHaikuPointerData touch_info;
...
G_DEFINE_TYPE (GdkHaikuSeat, gdk_haiku_seat, GDK_TYPE_SEAT)

struct _GdkHaikuDevice
{
  GdkDevice parent_instance;
  GdkHaikuTouchData *emulating_touch; /* Only used on wd->touch_master */
  GdkHaikuPointerData *pointer;
};
...
G_DEFINE_TYPE (GdkHaikuDevice, gdk_haiku_device, GDK_TYPE_DEVICE)

struct _GdkHaikuDevicePad
{
  GdkHaikuDevice parent_instance;
};

struct _GdkHaikuDevicePadClass
{
  GdkHaikuDeviceClass parent_class;
};

static void gdk_haiku_device_pad_iface_init (GdkDevicePadInterface *iface);

G_DEFINE_TYPE_WITH_CODE (GdkHaikuDevicePad, gdk_haiku_device_pad,
                         GDK_TYPE_HAIKU_DEVICE,
                         G_IMPLEMENT_INTERFACE (GDK_TYPE_DEVICE_PAD,
                                                gdk_haiku_device_pad_iface_init))

#define GDK_TYPE_HAIKU_DEVICE_PAD (gdk_haiku_device_pad_get_type ())
...
static void init_pointer_data (GdkHaikuPointerData *pointer_data,
                               GdkDisplay            *display_haiku,
                               GdkDevice             *master);
...
static void deliver_key_event (GdkHaikuSeat       *seat,
                               uint32_t              time_,
                               uint32_t              key,
                               uint32_t              state,
                               gboolean              from_key_repeat);
GType gdk_haiku_device_manager_get_type (void);

G_DEFINE_TYPE (GdkHaikuDeviceManager,
	       gdk_haiku_device_manager, GDK_TYPE_DEVICE_MANAGER)

static gboolean
gdk_haiku_device_get_history (GdkDevice      *device,
                                GdkWindow      *window,
                                guint32         start,
                                guint32         stop,
                                GdkTimeCoord ***events,
...
static void
gdk_haiku_device_get_state (GdkDevice       *device,
                              GdkWindow       *window,
                              gdouble         *axes,
...
static void
gdk_haiku_pointer_stop_cursor_animation (GdkHaikuPointerData *pointer)
{
...
static GdkHaikuTabletData *
gdk_haiku_device_manager_find_tablet (GdkHaikuSeat *seat,
                                        GdkDevice      *device)
{
  GList *l;
  ...
  for (l = seat->tablets; l; l = l->next)
    {
      GdkHaikuTabletData *tablet = l->data;
...
static GdkHaikuTabletPadData *
gdk_haiku_device_manager_find_pad (GdkHaikuSeat *seat,
                                     GdkDevice      *device)
{
  GList *l;
  ...
  for (l = seat->tablet_pads; l; l = l->next)
    {
      GdkHaikuTabletPadData *pad = l->data;
...
static unsigned int
_gdk_haiku_cursor_get_shape (GdkCursor *cursor,
                               int        version)
{
  gsize i;
  const char *cursor_name;

  cursor_name = _gdk_haiku_cursor_get_name (cursor);
  if (cursor_name == NULL ||
...
static gboolean
gdk_haiku_device_update_window_cursor (GdkDevice *device)
{
  GdkHaikuSeat *seat = GDK_HAIKU_SEAT (gdk_device_get_seat (device));
  GdkHaikuPointerData *pointer = GDK_HAIKU_DEVICE (device)->pointer;
  struct wl_buffer *buffer;
  int x, y, w, h, scale;
  guint next_image_index, next_image_delay;
  gboolean retval = G_SOURCE_REMOVE;
  GdkHaikuTabletData *tablet;
  unsigned int shape;

  tablet = gdk_haiku_device_manager_find_tablet (seat, device);
  ...
  if (GDK_HAIKU_DISPLAY (seat->display)->cursor_shape)
    {
      shape = _gdk_haiku_cursor_get_shape (pointer->cursor,
                                             wp_cursor_shape_manager_v1_get_version (GDK_HAIKU_DISPLAY (seat->display)->cursor_shape));
      if (shape != 0)
        {
  ...
  if (pointer->cursor)
    {
      buffer = _gdk_haiku_cursor_get_buffer (pointer->cursor,
  ...
  next_image_index =
    _gdk_haiku_cursor_get_next_image_index (pointer->cursor,
                                              pointer->cursor_image_index,
                                              &next_image_delay);
  ...
  if (next_image_index != pointer->cursor_image_index)
    {
      if (next_image_delay != pointer->cursor_image_delay ||
          pointer->cursor_timeout_id == 0)
        {
          ...
          gdk_haiku_pointer_stop_cursor_animation (pointer);

          /* Queue timeout for next frame */
          id = g_timeout_add (next_image_delay,
                              (GSourceFunc) gdk_haiku_device_update_window_cursor,
                              device);
          g_source_set_name_by_id (id, "[gtk+] gdk_haiku_device_update_window_cursor");
...
static void
gdk_haiku_device_set_window_cursor (GdkDevice *device,
                                      GdkWindow *window,
                                      GdkCursor *cursor)
{
  GdkHaikuSeat *seat = GDK_HAIKU_SEAT (gdk_device_get_seat (device));
  ...
  if (!cursor)
    {
      ...
      cursor =
        _gdk_haiku_display_get_cursor_for_name_with_scale (seat->display,
                                                             "default",
                                                             scale);
    }
  else
    _gdk_haiku_cursor_set_scale (cursor, pointer->current_output_scale);
  ...
  gdk_haiku_device_update_window_cursor (device);
}

static void
gdk_haiku_device_warp (GdkDevice *device,
                         GdkScreen *screen,
                         gdouble    x,
                         gdouble    y)
{
...
static void
gdk_haiku_device_query_state (GdkDevice        *device,
                                GdkWindow        *window,
                                ...
                                GdkModifierType  *mask)
{
  ...
  seat = GDK_HAIKU_SEAT (gdk_device_get_seat (device));
  pointer = GDK_HAIKU_DEVICE (device)->pointer;
                  ...
                  guint32          time_)
{
  ...
  _gdk_haiku_display_deliver_event (gdk_window_get_display (window), event);
}
...
GdkWindow *
gdk_haiku_device_get_focus (GdkDevice *device)
{
  GdkHaikuSeat *haiku_seat = GDK_HAIKU_SEAT (gdk_device_get_seat (device));
  GdkHaikuPointerData *pointer;

  if (device == haiku_seat->master_keyboard)
    return haiku_seat->keyboard_focus;
  else
    {
      pointer = GDK_HAIKU_DEVICE (device)->pointer;
...
device_maybe_emit_grab_crossing (GdkDevice *device,
                                 ...
                                 guint32    time)
{
  GdkWindow *native = gdk_haiku_device_get_focus (device);
                                   ...
                                   guint32         time)
{
  ...
  focus = gdk_haiku_device_get_focus (device);
...
gdk_haiku_device_grab (GdkDevice    *device,
                         GdkWindow    *window,
  ...
  if (gdk_device_get_source (device) == GDK_SOURCE_KEYBOARD)
    {
      ...
      if (gdk_window_get_window_type (window) == GDK_WINDOW_TOPLEVEL)
        {
          gdk_haiku_window_inhibit_shortcuts (window,
  ...
  else
    {
      ...
      _gdk_haiku_window_set_grab_seat (window, GDK_SEAT (haiku_seat));

      g_clear_object (&haiku_seat->cursor);

      if (cursor)
        haiku_seat->cursor = g_object_ref (cursor);
...
static void
gdk_haiku_device_ungrab (GdkDevice *device,
                           guint32    time_)
{
  GdkHaikuPointerData *pointer = GDK_HAIKU_DEVICE (device)->pointer;
  ...
  if (gdk_device_get_source (device) == GDK_SOURCE_KEYBOARD)
    {
      /* Device is a keyboard */
      if (prev_focus)
        gdk_haiku_window_restore_shortcuts (prev_focus,
                                              gdk_device_get_seat (device));
    }
  else
    {
      ...
      if (pointer->grab_window)
        _gdk_haiku_window_set_grab_seat (pointer->grab_window,
                                           NULL);
    }
}

static GdkWindow *
gdk_haiku_device_window_at_position (GdkDevice       *device,
                                       gdouble         *win_x,
                                       gdouble         *win_y,
                                       GdkModifierType *mask,
...
static void
gdk_haiku_device_select_window_events (GdkDevice    *device,
                                         GdkWindow    *window,
                                         GdkEventMask  event_mask)
{
}
...
static void
gdk_haiku_device_class_init (GdkHaikuDeviceClass *klass)
{
  GdkDeviceClass *device_class = GDK_DEVICE_CLASS (klass);

  device_class->get_history = gdk_haiku_device_get_history;
  device_class->get_state = gdk_haiku_device_get_state;
  device_class->set_window_cursor = gdk_haiku_device_set_window_cursor;
  device_class->warp = gdk_haiku_device_warp;
  device_class->query_state = gdk_haiku_device_query_state;
  device_class->grab = gdk_haiku_device_grab;
  device_class->ungrab = gdk_haiku_device_ungrab;
  device_class->window_at_position = gdk_haiku_device_window_at_position;
  device_class->select_window_events = gdk_haiku_device_select_window_events;
}

static void
gdk_haiku_device_init (GdkHaikuDevice *device_core)
{
  GdkDevice *device;
...
static gint
gdk_haiku_device_pad_get_n_groups (GdkDevicePad *pad)
{
  GdkSeat *seat = gdk_device_get_seat (GDK_DEVICE (pad));
  GdkHaikuTabletPadData *data;

  data = gdk_haiku_device_manager_find_pad (GDK_HAIKU_SEAT (seat),
                                              GDK_DEVICE (pad));
  g_assert (data != NULL);
...
static gint
gdk_haiku_device_pad_get_group_n_modes (GdkDevicePad *pad,
                                          gint          n_group)
{
  GdkSeat *seat = gdk_device_get_seat (GDK_DEVICE (pad));
  GdkHaikuTabletPadGroupData *group;
  GdkHaikuTabletPadData *data;
...
static gint
gdk_haiku_device_pad_get_n_features (GdkDevicePad        *pad,
...
static gint
gdk_haiku_device_pad_get_feature_group (GdkDevicePad        *pad,
                                          GdkDevicePadFeature  feature,
                                          gint                 idx)
{
  GdkSeat *seat = gdk_device_get_seat (GDK_DEVICE (pad));
...
static void
gdk_haiku_device_pad_iface_init (GdkDevicePadInterface *iface)
{
  iface->get_n_groups = gdk_haiku_device_pad_get_n_groups;
  iface->get_group_n_modes = gdk_haiku_device_pad_get_group_n_modes;
  iface->get_n_features = gdk_haiku_device_pad_get_n_features;
  iface->get_feature_group = gdk_haiku_device_pad_get_feature_group;
}

static void
gdk_haiku_device_pad_class_init (GdkHaikuDevicePadClass *klass)
{
}

static void
gdk_haiku_device_pad_init (GdkHaikuDevicePad *pad)
{
}

/**
 * gdk_haiku_device_get_wl_seat:
 * @device: (type GdkHaikuDevice): a #GdkDevice
 *
 ...
 */
struct wl_seat *
gdk_haiku_device_get_wl_seat (GdkDevice *device)
{
  GdkHaikuSeat *seat;

  g_return_val_if_fail (GDK_IS_HAIKU_DEVICE (device), NULL);
...
/**
 * gdk_haiku_device_get_wl_pointer:
 * @device: (type GdkHaikuDevice): a #GdkDevice
 *
 ...
 */
struct wl_pointer *
gdk_haiku_device_get_wl_pointer (GdkDevice *device)
{
  GdkHaikuSeat *seat;
...
/**
 * gdk_haiku_device_get_wl_keyboard:
 * @device: (type GdkHaikuDevice): a #GdkDevice
 *
 ...
 */
struct wl_keyboard *
gdk_haiku_device_get_wl_keyboard (GdkDevice *device)
{
  GdkHaikuSeat *seat;
...
GdkKeymap *
_gdk_haiku_device_get_keymap (GdkDevice *device)
{
  GdkHaikuSeat *seat = GDK_HAIKU_SEAT (gdk_device_get_seat (device));
  return seat->keymap;
}
...
{
  GdkHaikuSeat *seat = data;

  GDK_NOTE (EVENTS,
            g_message ("data device data offer, data device %p, offer %p",
                       data_device, offer));

  gdk_haiku_selection_ensure_offer (seat->display, offer);
}
                   ...
                   struct wl_data_offer  *offer)
{
  ...
  gdk_haiku_selection_set_offer (seat->display, selection, offer);

  gdk_haiku_drop_context_update_targets (seat->drop_context);

  _gdk_haiku_drag_context_set_source_window (seat->drop_context, dnd_owner);

  _gdk_haiku_drag_context_set_dest_window (seat->drop_context,
                                             dest_window, serial);
  _gdk_haiku_drag_context_set_coords (seat->drop_context,
                                        wl_fixed_to_double (x),
                                        wl_fixed_to_double (y));
  _gdk_haiku_drag_context_emit_event (seat->drop_context, GDK_DRAG_ENTER,
                                        GDK_CURRENT_TIME);
                   ...
                   struct wl_data_device *data_device)
{
  ...
  _gdk_haiku_drag_context_set_coords (seat->drop_context, -1, -1);
  _gdk_haiku_drag_context_emit_event (seat->drop_context, GDK_DRAG_LEAVE,
                                        GDK_CURRENT_TIME);
  _gdk_haiku_drag_context_set_dest_window (seat->drop_context, NULL, 0);
}
                    ...
                    wl_fixed_t             y)
{
  ...
  _gdk_haiku_drag_context_set_coords (seat->drop_context,
                                        wl_fixed_to_double (x),
                                        wl_fixed_to_double (y));
  _gdk_haiku_drag_context_emit_event (seat->drop_context,
                              ...
                              gpointer  primary_offer)
{
  ...
  gdk_haiku_selection_ensure_primary_offer (seat->display, primary_offer);
}
                             ...
                             gpointer  primary_offer)
{
  ...
  gdk_haiku_selection_set_offer (seat->display, selection, primary_offer);
  emit_selection_owner_change (seat->keyboard_focus, selection);
}
...
static GdkDevice * get_scroll_device (GdkHaikuSeat              *seat,
                                      enum wl_pointer_axis_source  source);

static GdkEvent *
create_scroll_event (GdkHaikuSeat        *seat,
                     GdkHaikuPointerData *pointer_info,
                     GdkDevice             *device,
                     GdkDevice             *source_device,
                     gboolean               emulated)
{
  GdkHaikuDisplay *display = GDK_HAIKU_DISPLAY (seat->display);
  GdkEvent *event;
...
static void
flush_discrete_scroll_event (GdkHaikuSeat     *seat,
                             GdkScrollDirection  direction)
{
  ...
  _gdk_haiku_display_deliver_event (seat->display, event);
}

static void
flush_smooth_scroll_event (GdkHaikuSeat *seat,
                           gdouble         delta_x,
                           gdouble         delta_y,
                           gboolean        is_stop)
{
...
static void
flush_scroll_event (GdkHaikuSeat             *seat,
                    GdkHaikuPointerFrameData *pointer_frame)
{
...
static void
gdk_haiku_seat_flush_frame_event (GdkHaikuSeat *seat)
{
  if (seat->pointer_info.frame.event)
    {
      _gdk_haiku_display_deliver_event (gdk_seat_get_display (GDK_SEAT (seat)),
...
static GdkEvent *
gdk_haiku_seat_get_frame_event (GdkHaikuSeat *seat,
                                  GdkEventType    evtype)
{
  if (seat->pointer_info.frame.event &&
      seat->pointer_info.frame.event->type != evtype)
    gdk_haiku_seat_flush_frame_event (seat);
                      ...
                      wl_fixed_t         sy)
{
  ...
  GdkHaikuDisplay *display_haiku =
    GDK_HAIKU_DISPLAY (seat->display);
  ...
  event = gdk_haiku_seat_get_frame_event (seat, GDK_ENTER_NOTIFY);
  ...
  gdk_haiku_device_update_window_cursor (seat->master_pointer);
  ...
  if (display_haiku->seat_version < WL_POINTER_HAS_FRAME)
    gdk_haiku_seat_flush_frame_event (seat);
}
                      ...
                      struct wl_surface *surface)
{
  GdkHaikuSeat *seat = data;
  GdkEvent *event;
  GdkHaikuDisplay *display_haiku = GDK_HAIKU_DISPLAY (seat->display);
  ...
  event = gdk_haiku_seat_get_frame_event (seat, GDK_LEAVE_NOTIFY);
  event->crossing.window = g_object_ref (seat->pointer_info.focus);
  gdk_event_set_device (event, seat->master_pointer);
  gdk_event_set_source_device (event, seat->pointer);
  ...
  seat->pointer_info.focus = NULL;
  if (seat->cursor)
    gdk_haiku_pointer_stop_cursor_animation (&seat->pointer_info);
                       ...
                       wl_fixed_t         sy)
{
  ...
  event = gdk_haiku_seat_get_frame_event (seat, GDK_MOTION_NOTIFY);
                       ...
                       uint32_t           state)
{
  ...
  _gdk_haiku_display_update_serial (display, serial);
  ...
  event = gdk_haiku_seat_get_frame_event (seat,
...
{
  GdkHaikuSeat *seat = data;
  GdkHaikuPointerFrameData *pointer_frame = &seat->pointer_info.frame;
  GdkHaikuDisplay *display = GDK_HAIKU_DISPLAY (seat->display);
                        ...
                        uint32_t            size)
{
  ...
  _gdk_haiku_keymap_update_from_fd (seat->keymap, format, fd, size);
...
static void stop_key_repeat (GdkHaikuSeat *seat);
...
static GSettings *
get_keyboard_settings (GdkHaikuSeat *seat)
{
...
static gboolean
get_key_repeat (GdkHaikuSeat *seat,
                guint          *delay,
                guint          *interval)
{
  gboolean repeat;
...
static void
stop_key_repeat (GdkHaikuSeat *seat)
{
...
static void
deliver_key_event (GdkHaikuSeat *seat,
                   uint32_t        time_,
                   uint32_t        key,
                   uint32_t        state,
                   gboolean        from_key_repeat)
{
  ...
  keymap = seat->keymap;
  xkb_state = _gdk_haiku_keymap_get_xkb_state (keymap);
  xkb_keymap = _gdk_haiku_keymap_get_xkb_keymap (keymap);
  ...
  event->key.is_modifier = _gdk_haiku_keymap_key_is_modifier (keymap, key);
...
static GdkHaikuTouchData *
gdk_haiku_seat_add_touch (GdkHaikuSeat    *seat,
                            uint32_t           id,
                            struct wl_surface *surface)
{
  GdkHaikuTouchData *touch;

  touch = g_new0 (GdkHaikuTouchData, 1);
  touch->id = id;
...
static GdkHaikuTouchData *
gdk_haiku_seat_get_touch (GdkHaikuSeat *seat,
                            uint32_t        id)
{
...
static void
gdk_haiku_seat_remove_touch (GdkHaikuSeat *seat,
                               uint32_t        id)
{
...
static GdkEvent *
_create_touch_event (GdkHaikuSeat       *seat,
                     GdkHaikuTouchData  *touch,
                     GdkEventType          evtype,
                     uint32_t              time)
{
...
static void
touch_handle_master_pointer_crossing (GdkHaikuSeat      *seat,
                                      GdkHaikuTouchData *touch,
                                      uint32_t             time)
{
  GdkHaikuPointerData *pointer;

  pointer = GDK_HAIKU_DEVICE (seat->touch_master)->pointer;
                   ...
                   wl_fixed_t         y)
{
  ...
  touch = gdk_haiku_seat_add_touch (seat, id, wl_surface);
  ...
  if (touch->initial_touch)
    {
      ...
      GDK_HAIKU_DEVICE(seat->touch_master)->emulating_touch = touch;
                 ...
                 int32_t          id)
{
  ...
  touch = gdk_haiku_seat_get_touch (seat, id);
  if (!touch)
    return;
  ...
  if (touch->initial_touch)
    GDK_HAIKU_DEVICE(seat->touch_master)->emulating_touch = NULL;

  gdk_haiku_seat_remove_touch (seat, id);
}
...
touch_handle_cancel (void            *data,
                     struct wl_touch *wl_touch)
{
  GdkHaikuSeat *haiku_seat = data;
  GdkHaikuTouchData *touch;
  GHashTableIter iter;
  GdkEvent *event;

  if (GDK_HAIKU_DEVICE (haiku_seat->touch_master)->emulating_touch)
    {
      touch = GDK_HAIKU_DEVICE (haiku_seat->touch_master)->emulating_touch;
      GDK_HAIKU_DEVICE (haiku_seat->touch_master)->emulating_touch = NULL;
    }

  g_hash_table_iter_init (&iter, haiku_seat->touches);

  while (g_hash_table_iter_next (&iter, NULL, (gpointer *) &touch))
    {
      event = _create_touch_event (haiku_seat, touch, GDK_TOUCH_CANCEL,
                                   GDK_CURRENT_TIME);
      _gdk_haiku_display_deliver_event (haiku_seat->display, event);
...
static void
emit_gesture_swipe_event (GdkHaikuSeat          *seat,
...
static void
emit_gesture_pinch_event (GdkHaikuSeat          *seat,
                          GdkTouchpadGesturePhase  phase,
                          guint32                  _time,
                          guint                    n_fingers,
                          gdouble                  dx,
...
static GdkDevice *
tablet_select_device_for_tool (GdkHaikuTabletData *tablet,
                               GdkDeviceTool        *tool)
{
  GdkDevice *device;
...
static void
_gdk_haiku_seat_remove_tool (GdkHaikuSeat           *seat,
                               GdkHaikuTabletToolData *tool)
{
...
static void
_gdk_haiku_seat_remove_tablet (GdkHaikuSeat       *seat,
                                 GdkHaikuTabletData *tablet)
{
  GdkHaikuDeviceManager *device_manager =
    GDK_HAIKU_DEVICE_MANAGER (seat->device_manager);
  ...
  while (tablet->pads)
    {
      GdkHaikuTabletPadData *pad = tablet->pads->data;
...
static void
_gdk_haiku_seat_remove_tablet_pad (GdkHaikuSeat          *seat,
                                     GdkHaikuTabletPadData *pad)
{
  GdkHaikuDeviceManager *device_manager =
    GDK_HAIKU_DEVICE_MANAGER (seat->device_manager);
...
static GdkHaikuTabletPadGroupData *
tablet_pad_lookup_button_group (GdkHaikuTabletPadData *pad,
                                uint32_t                 button)
{
  GdkHaikuTabletPadGroupData *group;
  GList *l;
...
tablet_handle_name (void                 *data,
                    ...
                    const char           *name)
{
  GdkHaikuTabletData *tablet = data;
...
{
  GdkHaikuTabletData *tablet = data;
  GdkHaikuSeat *seat = GDK_HAIKU_SEAT (tablet->seat);
  ...
  master = g_object_new (GDK_TYPE_HAIKU_DEVICE,
                         "name", master_name,
                         ...
                         "seat", seat,
                         NULL);
  GDK_HAIKU_DEVICE (master)->pointer = &tablet->pointer_info;

  eraser_name = g_strconcat (tablet->name, " (Eraser)", NULL);

  stylus_device = g_object_new (GDK_TYPE_HAIKU_DEVICE,
                                "name", tablet->name,
  ...
  eraser_device = g_object_new (GDK_TYPE_HAIKU_DEVICE,
                                "name", eraser_name,
                                "type", GDK_DEVICE_TYPE_SLAVE,
                       ...
                       struct zwp_tablet_v2 *wp_tablet)
{
  ...
  _gdk_haiku_seat_remove_tablet (GDK_HAIKU_SEAT (tablet->seat), tablet);
}
...
{
  GdkHaikuSeat *seat = data;
  GdkHaikuDeviceManager *device_manager = GDK_HAIKU_DEVICE_MANAGER (seat->device_manager);
  GdkHaikuDisplay *display_haiku = GDK_HAIKU_DISPLAY (seat->display);
  ...
  if ((caps & WL_SEAT_CAPABILITY_POINTER) && !seat->wl_pointer)
    {
      ...
      seat->pointer = g_object_new (GDK_TYPE_HAIKU_DEVICE,
      ...
      if (display_haiku->pointer_gestures)
        {
          seat->wp_pointer_gesture_swipe =
            zwp_pointer_gestures_v1_get_swipe_gesture (display_haiku->pointer_gestures,
                                                       seat->wl_pointer);
          ...
          seat->wp_pointer_gesture_pinch =
            zwp_pointer_gestures_v1_get_pinch_gesture (display_haiku->pointer_gestures,
                                                       seat->wl_pointer);
      ...
      if (display_haiku->cursor_shape)
        {
          seat->pointer_info.shape_device =
              wp_cursor_shape_manager_v1_get_pointer (display_haiku->cursor_shape, seat->wl_pointer);
        }
  ...
  if ((caps & WL_SEAT_CAPABILITY_KEYBOARD) && !seat->wl_keyboard)
    {
      ...
      seat->keyboard = g_object_new (GDK_TYPE_HAIKU_DEVICE,
  ...
  if ((caps & WL_SEAT_CAPABILITY_TOUCH) && !seat->wl_touch)
    {
      ...
      seat->touch_master = g_object_new (GDK_TYPE_HAIKU_DEVICE,
                                         ...
                                         "seat", seat,
                                         NULL);
      GDK_HAIKU_DEVICE (seat->touch_master)->pointer = &seat->touch_info;
      ...
      seat->touch = g_object_new (GDK_TYPE_HAIKU_DEVICE,
                                  "name", "Wayland Touch",
                                  "type", GDK_DEVICE_TYPE_SLAVE,
...
static GdkDevice *
get_scroll_device (GdkHaikuSeat              *seat,
                   enum wl_pointer_axis_source  source)
{
  GdkHaikuDeviceManager *device_manager = GDK_HAIKU_DEVICE_MANAGER (seat->device_manager);
  ...
  switch (source)
    ...
    case WL_POINTER_AXIS_SOURCE_WHEEL:
      if (seat->wheel_scrolling == NULL)
        {
          seat->wheel_scrolling = g_object_new (GDK_TYPE_HAIKU_DEVICE,
    ...
    case WL_POINTER_AXIS_SOURCE_FINGER:
      if (seat->finger_scrolling == NULL)
        {
          seat->finger_scrolling = g_object_new (GDK_TYPE_HAIKU_DEVICE,
    ...
    case WL_POINTER_AXIS_SOURCE_CONTINUOUS:
      if (seat->continuous_scrolling == NULL)
        {
          seat->continuous_scrolling = g_object_new (GDK_TYPE_HAIKU_DEVICE,
...
tablet_tool_handle_type (void                      *data,
                         ...
                         uint32_t                   tool_type)
{
  GdkHaikuTabletToolData *tool = data;
                            ...
                            struct zwp_tablet_tool_v2 *wp_tablet_tool)
{
  ...
  _gdk_haiku_seat_remove_tool (GDK_HAIKU_SEAT (tool->seat), tool);
}

static void
gdk_haiku_tablet_flush_frame_events (GdkHaikuTabletData *tablet,
                                       guint32               time)
{
  ...
  while (events)
    {
      ...
      _gdk_haiku_display_deliver_event (gdk_seat_get_display (tablet->seat),
                                          event);
...
static GdkEvent *
gdk_haiku_tablet_get_frame_event (GdkHaikuTabletData *tablet,
                                    GdkEventType          evtype)
{
  if (tablet->pointer_info.frame.event)
    {
...
static void
gdk_haiku_device_tablet_clone_tool_axes (GdkHaikuTabletData *tablet,
                                           GdkDeviceTool        *tool)
{
  gint axis_pos;
...
static void
gdk_haiku_mimic_device_axes (GdkDevice *master,
                               GdkDevice *slave)
{
...
{
  GdkHaikuTabletToolData *tool = data;
  GdkHaikuTabletData *tablet = zwp_tablet_v2_get_user_data (wp_tablet);
  GdkHaikuSeat *seat = GDK_HAIKU_SEAT (tablet->seat);
  GdkHaikuDisplay *display_haiku = GDK_HAIKU_DISPLAY (seat->display);
  GdkWindow *window;
  GdkEvent *event;
  ...
  gdk_haiku_device_tablet_clone_tool_axes (tablet, tool->tool);
  gdk_haiku_mimic_device_axes (tablet->master, tablet->current_device);

  event = gdk_haiku_tablet_get_frame_event (tablet, GDK_PROXIMITY_IN);
...
{
  GdkHaikuTabletToolData *tool = data;
  GdkHaikuTabletData *tablet = tool->current_tablet;
  GdkEvent *event;
#ifdef G_ENABLE_DEBUG
  GdkHaikuSeat *seat = GDK_HAIKU_SEAT (tool->seat);
#endif
  ...
  event = gdk_haiku_tablet_get_frame_event (tablet, GDK_PROXIMITY_OUT);
  event->proximity.window = g_object_ref (tablet->pointer_info.focus);
  gdk_event_set_device (event, tablet->master);
  gdk_event_set_source_device (event, tablet->current_device);
  gdk_event_set_device_tool (event, tool->tool);

  gdk_haiku_pointer_stop_cursor_animation (&tablet->pointer_info);

  gdk_haiku_device_update_window_cursor (tablet->master);
  g_object_unref (tablet->pointer_info.focus);
...
static void
tablet_create_button_event_frame (GdkHaikuTabletData *tablet,
                                  GdkEventType          evtype,
                                  guint                 button)
{
  GdkHaikuSeat *seat = GDK_HAIKU_SEAT (tablet->seat);
  ...
  event = gdk_haiku_tablet_get_frame_event (tablet, evtype);
  ...
  gdk_event_set_screen (event, display_haiku->screen);
                           ...
                           wl_fixed_t                 sy)
{
  ...
  event = gdk_haiku_tablet_get_frame_event (tablet, GDK_MOTION_NOTIFY);
                          ...
                          int32_t                    clicks)
{
  ...
  seat = GDK_HAIKU_SEAT (tablet->seat);
                          ...
                          uint32_t                   time)
{
  ...
  gdk_haiku_tablet_flush_frame_events (tablet, time);
}
...
tablet_pad_ring_handle_source (void                          *data,
                               ...
                               uint32_t                       source)
{
  GdkHaikuTabletPadGroupData *group = data;
...
{
  GdkHaikuTabletPadGroupData *group = data;
  GdkHaikuTabletPadData *pad = group->pad;
  GdkHaikuSeat *seat = GDK_HAIKU_SEAT (pad->seat);
  GdkEvent *event;
  ...
  _gdk_haiku_display_deliver_event (gdk_seat_get_display (pad->seat),
                                      event);
}
...
tablet_pad_handle_group (void                           *data,
                         ...
                         struct zwp_tablet_pad_group_v2 *wp_tablet_pad_group)
{
  GdkHaikuTabletPadData *pad = data;
  GdkHaikuTabletPadGroupData *group;
  ...
  group = g_new0 (GdkHaikuTabletPadGroupData, 1);
  ...
  if (!pad->device)
    {
      ...
      pad->device =
        g_object_new (GDK_TYPE_HAIKU_DEVICE_PAD,
                      "name", name,
                      "vendor-id", vid,
                      "product-id", pid,
                      "type", GDK_DEVICE_TYPE_SLAVE,
                           ...
                           struct zwp_tablet_pad_v2 *wp_tablet_pad)
{
  ...
  _gdk_haiku_seat_remove_tablet_pad (GDK_HAIKU_SEAT (pad->seat), pad);
}
                                 ...
                                 struct zwp_tablet_v2      *wp_tablet)
{
  ...
  tablet = g_new0 (GdkHaikuTabletData, 1);
                               ...
                               struct zwp_tablet_tool_v2 *wp_tablet_tool)
{
  ...
  tool = g_new0 (GdkHaikuTabletToolData, 1);
                              ...
                              struct zwp_tablet_pad_v2  *wp_tablet_pad)
{
  ...
  pad = g_new0 (GdkHaikuTabletPadData, 1);
...
static void
init_devices (GdkHaikuSeat *seat)
{
  GdkHaikuDeviceManager *device_manager = GDK_HAIKU_DEVICE_MANAGER (seat->device_manager);
  GdkHaikuDisplay *display = GDK_HAIKU_DISPLAY (seat->display);

  /* pointer */
  seat->master_pointer = g_object_new (GDK_TYPE_HAIKU_DEVICE,
                                       "name", "Core Pointer",
                                       "type", GDK_DEVICE_TYPE_MASTER,
                                       "input-source", GDK_SOURCE_MOUSE,
                                       "input-mode", GDK_MODE_SCREEN,
  ...
  GDK_HAIKU_DEVICE (seat->master_pointer)->pointer = &seat->pointer_info;
  ...
  /* keyboard */
  seat->master_keyboard = g_object_new (GDK_TYPE_HAIKU_DEVICE,
                                        "name", "Core Keyboard",
                                        "type", GDK_DEVICE_TYPE_MASTER,
                                        "input-source", GDK_SOURCE_KEYBOARD,
                                        "input-mode", GDK_MODE_SCREEN,
...
pointer_surface_update_scale (GdkDevice *device)
{
  ...
  if (display_haiku->compositor_version < WL_SURFACE_HAS_BUFFER_SCALE)
    {
  ...
  for (l = pointer->pointer_surface_outputs; l != NULL; l = l->next)
    {
      guint32 output_scale =
        _gdk_haiku_screen_get_output_scale (display_haiku->screen,
                                              l->data);
  ...
  if (pointer->cursor)
    _gdk_haiku_cursor_set_scale (pointer->cursor, scale);
...
static void
gdk_haiku_pointer_data_finalize (GdkHaikuPointerData *pointer)
{
...
static void
gdk_haiku_seat_finalize (GObject *object)
{
  GdkHaikuSeat *seat = GDK_HAIKU_SEAT (object);
  GList *l;

  for (l = seat->tablet_tools; l != NULL; l = l->next)
    _gdk_haiku_seat_remove_tool (seat, l->data);

  for (l = seat->tablet_pads; l != NULL; l = l->next)
    _gdk_haiku_seat_remove_tablet_pad (seat, l->data);

  for (l = seat->tablets; l != NULL; l = l->next)
    _gdk_haiku_seat_remove_tablet (seat, l->data);

  seat_handle_capabilities (seat, seat->wl_seat, 0);
  g_object_unref (seat->keymap);
  gdk_haiku_pointer_data_finalize (&seat->pointer_info);
  ...
  G_OBJECT_CLASS (gdk_haiku_seat_parent_class)->finalize (object);
}

static GdkSeatCapabilities
gdk_haiku_seat_get_capabilities (GdkSeat *seat)
{
  GdkHaikuSeat *haiku_seat = GDK_HAIKU_SEAT (seat);
  GdkSeatCapabilities caps = 0;

  if (haiku_seat->master_pointer)
    caps |= GDK_SEAT_CAPABILITY_POINTER;
  if (haiku_seat->master_keyboard)
    caps |= GDK_SEAT_CAPABILITY_KEYBOARD;
  if (haiku_seat->touch_master)
...
static void
gdk_haiku_seat_set_grab_window (GdkHaikuSeat *seat,
                                  GdkWindow      *window)
{
  if (seat->grab_window)
    {
      _gdk_haiku_window_set_grab_seat (seat->grab_window, NULL);
  ...
  if (window)
    {
      ...
      g_object_add_weak_pointer (G_OBJECT (window),
                                 (gpointer *) &seat->grab_window);
      _gdk_haiku_window_set_grab_seat (window, GDK_SEAT (seat));
    }
}

static GdkGrabStatus
gdk_haiku_seat_grab (GdkSeat                *seat,
                       GdkWindow              *window,
                       ...
                       gpointer                prepare_func_data)
{
  ...
  gdk_haiku_seat_set_grab_window (haiku_seat, native);
  haiku_seat->grab_time = evtime;
  ...
  if (!gdk_window_is_visible (window))
    {
      gdk_haiku_seat_set_grab_window (haiku_seat, NULL);
  ...
  if (haiku_seat->master_pointer &&
    ...
    {
      device_maybe_emit_grab_crossing (haiku_seat->master_pointer,
                                       native, evtime);

      _gdk_display_add_device_grab (display,
                                    haiku_seat->master_pointer,
                                    window,
                                    native,
                                    GDK_OWNERSHIP_NONE,
                                    owner_events,
      ...
      gdk_haiku_seat_set_global_cursor (seat, cursor);
      g_set_object (&haiku_seat->cursor, cursor);
      gdk_haiku_device_update_window_cursor (haiku_seat->master_pointer);
    }

  if (haiku_seat->touch_master &&
      capabilities & GDK_SEAT_CAPABILITY_TOUCH)
    {
      device_maybe_emit_grab_crossing (haiku_seat->touch_master,
                                       native, evtime);

      _gdk_display_add_device_grab (display,
                                    haiku_seat->touch_master,
                                    window,
                                    native,
                                    GDK_OWNERSHIP_NONE,
                                    owner_events,
  ...
  if (haiku_seat->master_keyboard &&
    ...
    {
      device_maybe_emit_grab_crossing (haiku_seat->master_keyboard,
                                       native, evtime);

      _gdk_display_add_device_grab (display,
                                    haiku_seat->master_keyboard,
                                    window,
                                    native,
                                    GDK_OWNERSHIP_NONE,
                                    owner_events,
      ...
      if (capabilities == GDK_SEAT_CAPABILITY_KEYBOARD &&
        ...
        gdk_haiku_window_inhibit_shortcuts (window, seat);
    }

  if (haiku_seat->tablets &&
      capabilities & GDK_SEAT_CAPABILITY_TABLET_STYLUS)
    {
      for (l = haiku_seat->tablets; l; l = l->next)
        {
          GdkHaikuTabletData *tablet = l->data;
...
static void
gdk_haiku_seat_ungrab (GdkSeat *seat)
{
  GdkHaikuSeat *haiku_seat = GDK_HAIKU_SEAT (seat);
  GdkDisplay *display = gdk_seat_get_display (seat);
  GdkDeviceGrabInfo *grab;
  ...
  g_clear_object (&haiku_seat->grab_cursor);

  gdk_haiku_seat_set_grab_window (haiku_seat, NULL);

  if (haiku_seat->master_pointer)
    {
      device_maybe_emit_ungrab_crossing (haiku_seat->master_pointer,
                                         GDK_CURRENT_TIME);
  ...
  if (haiku_seat->master_keyboard)
    {
      ...
      prev_focus = device_maybe_emit_ungrab_crossing (haiku_seat->master_keyboard,
                                                      GDK_CURRENT_TIME);
      if (prev_focus)
        gdk_haiku_window_restore_shortcuts (prev_focus, seat);
    }

  if (haiku_seat->touch_master)
    {
      grab = _gdk_display_get_last_device_grab (display, haiku_seat->touch_master);
...
static GdkDevice *
gdk_haiku_seat_get_master (GdkSeat             *seat,
                             GdkSeatCapabilities  capabilities)
{
  GdkHaikuSeat *haiku_seat = GDK_HAIKU_SEAT (seat);

  if (capabilities == GDK_SEAT_CAPABILITY_POINTER)
    return haiku_seat->master_pointer;
  else if (capabilities == GDK_SEAT_CAPABILITY_KEYBOARD)
    return haiku_seat->master_keyboard;
  else if (capabilities == GDK_SEAT_CAPABILITY_TOUCH)
    return haiku_seat->touch_master;

  return NULL;
}

static GList *
gdk_haiku_seat_get_slaves (GdkSeat             *seat,
                             GdkSeatCapabilities  capabilities)
{
  GdkHaikuSeat *haiku_seat = GDK_HAIKU_SEAT (seat);
  GList *slaves = NULL;

  if (haiku_seat->finger_scrolling && (capabilities & GDK_SEAT_CAPABILITY_POINTER))
    slaves = g_list_prepend (slaves, haiku_seat->finger_scrolling);
  if (haiku_seat->continuous_scrolling && (capabilities & GDK_SEAT_CAPABILITY_POINTER))
    slaves = g_list_prepend (slaves, haiku_seat->continuous_scrolling);
  if (haiku_seat->wheel_scrolling && (capabilities & GDK_SEAT_CAPABILITY_POINTER))
    slaves = g_list_prepend (slaves, haiku_seat->wheel_scrolling);
  if (haiku_seat->pointer && (capabilities & GDK_SEAT_CAPABILITY_POINTER))
    slaves = g_list_prepend (slaves, haiku_seat->pointer);
  if (haiku_seat->keyboard && (capabilities & GDK_SEAT_CAPABILITY_KEYBOARD))
    slaves = g_list_prepend (slaves, haiku_seat->keyboard);
  if (haiku_seat->touch && (capabilities & GDK_SEAT_CAPABILITY_TOUCH))
    slaves = g_list_prepend (slaves, haiku_seat->touch);

  if (haiku_seat->tablets && (capabilities & GDK_SEAT_CAPABILITY_TABLET_STYLUS))
    {
      GList *l;
  ...
  if (capabilities == GDK_SEAT_CAPABILITY_ALL)
    {
      ...
      for (l = haiku_seat->tablet_pads; l; l = l->next)
        {
          GdkHaikuTabletPadData *pad = l->data;
...
static void
gdk_haiku_seat_class_init (GdkHaikuSeatClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GdkSeatClass *seat_class = GDK_SEAT_CLASS (klass);

  object_class->finalize = gdk_haiku_seat_finalize;

  seat_class->get_capabilities = gdk_haiku_seat_get_capabilities;
  seat_class->grab = gdk_haiku_seat_grab;
  seat_class->ungrab = gdk_haiku_seat_ungrab;
  seat_class->get_master = gdk_haiku_seat_get_master;
  seat_class->get_slaves = gdk_haiku_seat_get_slaves;
}

static void
gdk_haiku_seat_init (GdkHaikuSeat *seat)
{
}

static void
init_pointer_data (GdkHaikuPointerData *pointer_data,
                   GdkDisplay            *display,
                   GdkDevice             *master)
{
  GdkHaikuDisplay *display_haiku;
  ...
  pointer_data->pointer_surface =
    wl_compositor_create_surface (display_haiku->compositor);
...
void
_gdk_haiku_device_manager_add_seat (GdkDeviceManager *device_manager,
                                      guint32           id,
				      struct wl_seat   *wl_seat)
{
  GdkDisplay *display;
  ...
  seat = g_object_new (GDK_TYPE_HAIKU_SEAT,
                       "display", display,
                       NULL);
  seat->id = id;
  seat->keymap = _gdk_haiku_keymap_new ();
  ...
  if (display_haiku->zwp_primary_selection_manager_v1)
    {
      seat->zwp_primary_data_device_v1 =
        zwp_primary_selection_device_manager_v1_get_device (display_haiku->zwp_primary_selection_manager_v1,
                                                            seat->wl_seat);
      zwp_primary_selection_device_v1_add_listener (seat->zwp_primary_data_device_v1,
                                                    &zwp_primary_device_v1_listener,
                                                    seat);
    }
  else if (display_haiku->gtk_primary_selection_manager)
    {
      seat->gtk_primary_data_device =
        gtk_primary_selection_device_manager_get_device (display_haiku->gtk_primary_selection_manager,
                                                         seat->wl_seat);
  ...
  seat->data_device =
    wl_data_device_manager_get_data_device (display_haiku->data_device_manager,
                                            seat->wl_seat);
  seat->drop_context = _gdk_haiku_drop_context_new (display,
                                                      seat->data_device);
  ...
  if (display_haiku->tablet_manager)
    {
      seat->wp_tablet_seat =
        zwp_tablet_manager_v2_get_tablet_seat (display_haiku->tablet_manager,
                                               wl_seat);
...
void
_gdk_haiku_device_manager_remove_seat (GdkDeviceManager *manager,
                                         guint32           id)
{
...
gdk_haiku_device_manager_list_devices (GdkDeviceManager *device_manager,
                                         GdkDeviceType     type)
{
  GdkHaikuDeviceManager *haiku_device_manager;
  GList *devices = NULL, *l;

  haiku_device_manager = GDK_HAIKU_DEVICE_MANAGER (device_manager);

  for (l = haiku_device_manager->devices; l; l = l->next)
    {
...
gdk_haiku_device_manager_get_client_pointer (GdkDeviceManager *device_manager)
{
  ...
  /* Find the master pointer of the first GdkHaikuSeat we find */
  device = haiku_device_manager->devices->data;
  seat = GDK_HAIKU_SEAT (gdk_device_get_seat (device));
...
uint32_t
_gdk_haiku_device_get_implicit_grab_serial (GdkHaikuDevice *device,
                                              const GdkEvent   *event)
{
  GdkSeat *seat = gdk_device_get_seat (GDK_DEVICE (device));
  GdkEventSequence *sequence = NULL;
  GdkHaikuTouchData *touch = NULL;

  if (event)
    sequence = gdk_event_get_event_sequence (event);

  if (sequence)
    touch = gdk_haiku_seat_get_touch (GDK_HAIKU_SEAT (seat),
  ...
  if (event)
    ...
    return GDK_HAIKU_SEAT (seat)->pointer_info.press_serial;
}

uint32_t
_gdk_haiku_seat_get_last_implicit_grab_serial (GdkSeat           *seat,
                                                 GdkEventSequence **sequence)
{
  GdkHaikuSeat *haiku_seat;
  GdkHaikuTouchData *touch;
  GHashTableIter iter;
  GList *l;
  uint32_t serial;

  haiku_seat = GDK_HAIKU_SEAT (seat);
  g_hash_table_iter_init (&iter, haiku_seat->touches);

  if (sequence)
    *sequence = NULL;

  serial = haiku_seat->keyboard_key_serial;

  if (haiku_seat->pointer_info.press_serial > serial)
    serial = haiku_seat->pointer_info.press_serial;
...
void
gdk_haiku_device_unset_touch_grab (GdkDevice        *gdk_device,
                                     GdkEventSequence *sequence)
{
  GdkHaikuSeat *seat;
  GdkHaikuTouchData *touch;
  ...
  g_return_if_fail (GDK_IS_HAIKU_DEVICE (gdk_device));

  seat = GDK_HAIKU_SEAT (gdk_device_get_seat (gdk_device));
  touch = gdk_haiku_seat_get_touch (seat,
                                      GDK_EVENT_SEQUENCE_TO_SLOT (sequence));

  if (GDK_HAIKU_DEVICE (seat->touch_master)->emulating_touch == touch)
    {
      GDK_HAIKU_DEVICE (seat->touch_master)->emulating_touch = NULL;
      emulate_touch_crossing (touch->window, NULL,
...
void
gdk_haiku_seat_set_global_cursor (GdkSeat   *seat,
                                    GdkCursor *cursor)
{
  GdkHaikuSeat *haiku_seat = GDK_HAIKU_SEAT (seat);
  GdkDevice *pointer;
  ...
  g_set_object (&haiku_seat->grab_cursor, cursor);
  gdk_haiku_device_set_window_cursor (pointer,
                                        gdk_haiku_device_get_focus (pointer),
                                        NULL);
}

struct wl_data_device *
gdk_haiku_device_get_data_device (GdkDevice *gdk_device)
{
  GdkHaikuSeat *seat;

  g_return_val_if_fail (GDK_IS_HAIKU_DEVICE (gdk_device), NULL);
...
void
gdk_haiku_seat_set_selection (GdkSeat               *seat,
                                struct wl_data_source *source)
{
  GdkHaikuSeat *haiku_seat = GDK_HAIKU_SEAT (seat);
  GdkHaikuDisplay *display_haiku;

  display_haiku = GDK_HAIKU_DISPLAY (haiku_seat->display);

  wl_data_device_set_selection (haiku_seat->data_device, source,
                                _gdk_haiku_display_get_serial (display_haiku));
}

void
gdk_haiku_seat_set_primary (GdkSeat  *seat,
                              gpointer  source)
{
  GdkHaikuSeat *haiku_seat = GDK_HAIKU_SEAT (seat);
  GdkHaikuDisplay *display_haiku;
  ...
  if (source)
    {
      display_haiku = GDK_HAIKU_DISPLAY (gdk_seat_get_display (seat));
      serial = _gdk_haiku_display_get_serial (display_haiku);
      if (haiku_seat->zwp_primary_data_device_v1)
        {
          zwp_primary_selection_device_v1_set_selection (haiku_seat->zwp_primary_data_device_v1,
                                                         source, serial);
        }
      else if (haiku_seat->gtk_primary_data_device)
        {
          gtk_primary_selection_device_set_selection (haiku_seat->gtk_primary_data_device,
                                                      source, serial);
        }
    }
}
 ...
 */
struct wl_seat *
gdk_haiku_seat_get_wl_seat (GdkSeat *seat)
{
  g_return_val_if_fail (GDK_IS_HAIKU_SEAT (seat), NULL);

  return GDK_HAIKU_SEAT (seat)->wl_seat;
}

GdkDragContext *
gdk_haiku_device_get_drop_context (GdkDevice *device)
{
  GdkSeat *seat = gdk_device_get_seat (device);

  return GDK_HAIKU_SEAT (seat)->drop_context;
}

/**
 * gdk_haiku_device_get_node_path:
 * @device: a #GdkDevice
 *
 ...
 **/
const gchar *
gdk_haiku_device_get_node_path (GdkDevice *device)
{
  GdkHaikuTabletData *tablet;
  GdkHaikuTabletPadData *pad;
  ...
  tablet = gdk_haiku_device_manager_find_tablet (GDK_HAIKU_SEAT (seat),
                                                   device);
  if (tablet)
    return tablet->path;

  pad = gdk_haiku_device_manager_find_pad (GDK_HAIKU_SEAT (seat), device);
  if (pad)
    return pad->path;
...
/**
 * gdk_haiku_device_pad_set_feedback:
 ...
 **/
void
gdk_haiku_device_pad_set_feedback (GdkDevice           *device,
                                     GdkDevicePadFeature  feature,
                                     guint                feature_idx,
                                     const gchar         *label)
{
  ...
  seat = gdk_device_get_seat (device);
  pad = gdk_haiku_device_manager_find_pad (GDK_HAIKU_SEAT (seat),
                                             device);
  if (!pad)
    return;
