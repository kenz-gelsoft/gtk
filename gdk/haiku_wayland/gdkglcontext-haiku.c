 ...
 *
 * gdkglcontext-haiku.c: Wayland specific OpenGL wrappers
 *
                                             ...
                                             cairo_region_t *update_area)
{
  ...
  context_haiku = GDK_HAIKU_GL_CONTEXT (window->gl_paint_context);
  buffer_age = 0;

  egl_surface = gdk_haiku_window_get_egl_surface (window->impl_window,
                                                    context_haiku->egl_config);

  if (display_haiku->have_egl_buffer_age)
    {
      gdk_gl_context_make_current (window->gl_paint_context);
      eglQuerySurface (display_haiku->egl_display, egl_surface,
                                ...
                                GError      **error)
{
  ...
  ctx = eglCreateContext (display_haiku->egl_display,
                          context_haiku->egl_config,
                          share != NULL ? GDK_HAIKU_GL_CONTEXT (share)->egl_context
                                        : EGL_NO_CONTEXT,
                          context_attribs);
                                  ...
                                  cairo_region_t *damage)
{
  ...
  if (display_haiku->have_egl_swap_buffers_with_damage)
    {
      ...
      eglSwapBuffersWithDamageEXT (display_haiku->egl_display, egl_surface, rects, n_rects);
      g_free (rects);
    }
  else
    eglSwapBuffers (display_haiku->egl_display, egl_surface);
}
...
static EGLDisplay
gdk_haiku_get_display (GdkHaikuDisplay *display_haiku)
{
  EGLDisplay dpy = NULL;
  ...
  return eglGetDisplay ((EGLNativeDisplayType) display_haiku->wl_display);
}
                             ...
                             GdkGLContext *share)
{
  ...
  if (display_haiku->have_egl)
    return TRUE;

  dpy = gdk_haiku_get_display (display_haiku);
  ...
  display_haiku->egl_display = dpy;
  display_haiku->egl_major_version = major;
  display_haiku->egl_minor_version = minor;

  display_haiku->have_egl = TRUE;

  display_haiku->have_egl_khr_create_context =
    epoxy_has_egl_extension (dpy, "EGL_KHR_create_context");

  display_haiku->have_egl_buffer_age =
    epoxy_has_egl_extension (dpy, "EGL_EXT_buffer_age");

  display_haiku->have_egl_swap_buffers_with_damage =
    epoxy_has_egl_extension (dpy, "EGL_EXT_swap_buffers_with_damage");

  display_haiku->have_egl_surfaceless_context =
  ...
  GDK_NOTE (OPENGL,
            g_message ("EGL API version %d.%d found\n"
                       ...
                       " - Extensions:\n"
                       "\t%s",
                       display_haiku->egl_major_version,
                       display_haiku->egl_minor_version,
                           ...
                           GError    **error)
{
  ...
  if (!eglChooseConfig (display_haiku->egl_display, attrs, NULL, 0, &count) || count < 1)
    {
      g_set_error_literal (error, GDK_GL_ERROR,
  ...
  if (!eglChooseConfig (display_haiku->egl_display, attrs, configs, count, &count) || count < 1)
    {
      g_set_error_literal (error, GDK_GL_ERROR,
                           GDK_GL_ERROR_UNSUPPORTED_FORMAT,
                           _("No available configurations for the given pixel format"));
  ...
  if (!eglGetConfigAttrib (display_haiku->egl_display, chosen_config,
                                      ...
                                      GError       **error)
{
  ...
  if (!gdk_haiku_display_init_gl (display, share))
    {
      g_set_error_literal (error, GDK_GL_ERROR,
                           GDK_GL_ERROR_NOT_AVAILABLE,
  ...
  if (!display_haiku->have_egl_khr_create_context)
    {
      g_set_error_literal (error, GDK_GL_ERROR,
  ...
  if (!find_eglconfig_for_window (window, &config,
                                  &display_haiku->egl_min_swap_interval,
                                  error))
    return NULL;
...
gdk_x11_gl_context_dispose (GObject *gobject)
{
  ...
  if (context_haiku->egl_context != NULL)
    {
      ...
      if (eglGetCurrentContext () == context_haiku->egl_context)
        eglMakeCurrent(display_haiku->egl_display, EGL_NO_SURFACE, EGL_NO_SURFACE,
                       EGL_NO_CONTEXT);

      GDK_NOTE (OPENGL, g_message ("Destroying EGL context"));

      eglDestroyContext (display_haiku->egl_display,
                         context_haiku->egl_context);
      context_haiku->egl_context = NULL;
    }
                                           ...
                                           GdkGLContext *context)
{
  ...
  return context_haiku->egl_context == eglGetCurrentContext ();
}
                                             ...
                                             GdkGLContext *context)
{
  ...
  if (context_haiku->is_attached)
    egl_surface = gdk_haiku_window_get_egl_surface (window->impl_window, context_haiku->egl_config);
  else
    {
      if (display_haiku->have_egl_surfaceless_context)
	egl_surface = EGL_NO_SURFACE;
      else
	egl_surface = gdk_haiku_window_get_dummy_egl_surface (window->impl_window,
								context_haiku->egl_config);
    }

  if (!eglMakeCurrent (display_haiku->egl_display, egl_surface,
                       egl_surface, context_haiku->egl_context))
    {
      g_warning ("eglMakeCurrent failed");
      return FALSE;
    }

  if (display_haiku->egl_min_swap_interval == 0)
    eglSwapInterval (display_haiku->egl_display, 0);
  else
