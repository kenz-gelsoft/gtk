...
typedef struct _GdkHaikuScreenClass GdkHaikuScreenClass;

#define GDK_TYPE_HAIKU_SCREEN              (_gdk_haiku_screen_get_type ())
...
GType _gdk_haiku_screen_get_type (void);

G_DEFINE_TYPE (GdkHaikuScreen, _gdk_haiku_screen, GDK_TYPE_SCREEN)

static void
gdk_haiku_screen_dispose (GObject *object)
{
  GdkHaikuScreen *screen_haiku = GDK_HAIKU_SCREEN (object);

  if (screen_haiku->dbus_proxy && screen_haiku->dbus_setting_change_id > 0)
    {
      g_signal_handler_disconnect (screen_haiku->dbus_proxy,
                                   screen_haiku->dbus_setting_change_id);
      screen_haiku->dbus_setting_change_id = 0;
    }

  g_cancellable_cancel (screen_haiku->dbus_cancellable);

  if (screen_haiku->root_window)
    _gdk_window_destroy (screen_haiku->root_window, FALSE);

  G_OBJECT_CLASS (_gdk_haiku_screen_parent_class)->dispose (object);
}

static void
gdk_haiku_screen_finalize (GObject *object)
{
  GdkHaikuScreen *screen_haiku = GDK_HAIKU_SCREEN (object);

  g_clear_object (&screen_haiku->dbus_proxy);
  g_clear_object (&screen_haiku->dbus_cancellable);

  if (screen_haiku->root_window)
    g_object_unref (screen_haiku->root_window);

  g_object_unref (screen_haiku->visual);

  if (screen_haiku->settings)
    g_hash_table_destroy (screen_haiku->settings);

  g_clear_object (&screen_haiku->settings_portal);

  g_free (screen_haiku->dbus_settings.modules);

  G_OBJECT_CLASS (_gdk_haiku_screen_parent_class)->finalize (object);
}

static GdkDisplay *
gdk_haiku_screen_get_display (GdkScreen *screen)
{
  return GDK_HAIKU_SCREEN (screen)->display;
}
...
{
  return GDK_HAIKU_SCREEN (screen)->width_mm;
}

static gint
gdk_haiku_screen_get_height_mm (GdkScreen *screen)
{
  return GDK_HAIKU_SCREEN (screen)->height_mm;
}

static gint
gdk_haiku_screen_get_number (GdkScreen *screen)
{
  return 0;
}

static GdkWindow *
gdk_haiku_screen_get_root_window (GdkScreen *screen)
{
  return GDK_HAIKU_SCREEN (screen)->root_window;
}

static GdkVisual *
gdk_haiku_screen_get_system_visual (GdkScreen * screen)
{
  return (GdkVisual *) GDK_HAIKU_SCREEN (screen)->visual;
}

static GdkVisual *
gdk_haiku_screen_get_rgba_visual (GdkScreen *screen)
{
  return (GdkVisual *) GDK_HAIKU_SCREEN (screen)->visual;
}

static gboolean
gdk_haiku_screen_is_composited (GdkScreen *screen)
{
  return TRUE;
}

static gchar *
gdk_haiku_screen_make_display_name (GdkScreen *screen)
{
  return g_strdup (gdk_display_get_name (GDK_HAIKU_SCREEN (screen)->display));
}

static GdkWindow *
gdk_haiku_screen_get_active_window (GdkScreen *screen)
{
  return NULL;
}

static GList *
gdk_haiku_screen_get_window_stack (GdkScreen *screen)
{
  return NULL;
}

static void
gdk_haiku_screen_broadcast_client_message (GdkScreen *screen,
					     GdkEvent  *event)
{
}
...
static gdouble
get_dpi_from_gsettings (GdkHaikuScreen *screen_haiku)
{
  GSettings *settings;
  gdouble factor;

  settings = g_hash_table_lookup (screen_haiku->settings,
...
{
  GdkHaikuScreen *screen_haiku = GDK_HAIKU_SCREEN (screen);
  GSettings *settings;
  ...
  if (screen_haiku->settings_portal)
    {
  ...
  else
    {
      ...
      else if (g_hash_table_contains (screen_haiku->settings,
      ...
      dpi = get_dpi_from_gsettings (screen_haiku) * 1024;
    }
  ...
  if (screen_haiku->xft_settings.antialias != xft_settings.antialias)
    {
      screen_haiku->xft_settings.antialias = xft_settings.antialias;
      if (notify)
        notify_setting (screen, "gtk-xft-antialias");
    }

  if (screen_haiku->xft_settings.hinting != xft_settings.hinting)
    {
      screen_haiku->xft_settings.hinting = xft_settings.hinting;
      if (notify)
        notify_setting (screen, "gtk-xft-hinting");
    }

  if (screen_haiku->xft_settings.hintstyle != xft_settings.hintstyle)
    {
      screen_haiku->xft_settings.hintstyle = xft_settings.hintstyle;
      if (notify)
        notify_setting (screen, "gtk-xft-hintstyle");
    }

  if (screen_haiku->xft_settings.rgba != xft_settings.rgba)
    {
      screen_haiku->xft_settings.rgba = xft_settings.rgba;
      if (notify)
        notify_setting (screen, "gtk-xft-rgba");
    }

  if (screen_haiku->xft_settings.dpi != xft_settings.dpi)
    {
      double dpi = xft_settings.dpi / 1024.;
      const char *scale_env;
      double scale;

      screen_haiku->xft_settings.dpi = xft_settings.dpi;
  ...
  if (gdk_should_use_portal ())
    {
      ...
      screen_haiku->settings_portal = g_dbus_proxy_new_for_bus_sync (G_BUS_TYPE_SESSION,
      ...
      ret = g_dbus_proxy_call_sync (screen_haiku->settings_portal,
                                    "ReadAll",
      ...
      g_signal_connect (screen_haiku->settings_portal, "g-signal",
                        G_CALLBACK (settings_portal_changed), screen_haiku);
...
fallback:
  ...
  screen_haiku->dbus_cancellable = g_cancellable_new ();
  g_dbus_proxy_new_for_bus (G_BUS_TYPE_SESSION,
                            ...
                            GTK_SETTINGS_DBUS_NAME,
                            screen_haiku->dbus_cancellable,
                            fontconfig_dbus_proxy_open_cb,
                            screen_haiku);

  screen_haiku->settings = g_hash_table_new_full (g_str_hash, g_str_equal, NULL, g_object_unref);
  ...
  for (i = 0; i < G_N_ELEMENTS (translations); i++)
    {
      ...
      if (g_hash_table_lookup (screen_haiku->settings, (gpointer)translations[i].schema) == NULL)
        {
          settings = g_settings_new_full (schema, NULL, NULL);
          g_signal_connect (settings, "changed",
                            G_CALLBACK (settings_changed), screen);
          g_hash_table_insert (screen_haiku->settings, (gpointer)translations[i].schema, settings);
        }
                               ...
                               uint32_t           capabilities)
{
  ...
  GdkHaikuScreen *screen_haiku = GDK_HAIKU_SCREEN (data);

  screen_haiku->shell_capabilities = capabilities;
...
void
_gdk_haiku_screen_set_has_gtk_shell (GdkScreen *screen)
{
  GdkHaikuDisplay *display_haiku =
    GDK_HAIKU_DISPLAY (GDK_HAIKU_SCREEN (screen)->display);

  gtk_shell1_add_listener (display_haiku->gtk_shell,
                      ...
                      GValue           *value)
{
      ...
      switch (entry->type)
        ...
        case G_TYPE_NONE:
          if (g_str_equal (entry->setting, "gtk-xft-antialias"))
            g_value_set_int (value, screen_haiku->xft_settings.antialias);
          else if (g_str_equal (entry->setting, "gtk-xft-hinting"))
            g_value_set_int (value, screen_haiku->xft_settings.hinting);
          else if (g_str_equal (entry->setting, "gtk-xft-hintstyle"))
            g_value_set_static_string (value, screen_haiku->xft_settings.hintstyle);
          else if (g_str_equal (entry->setting, "gtk-xft-rgba"))
            g_value_set_static_string (value, screen_haiku->xft_settings.rgba);
          else if (g_str_equal (entry->setting, "gtk-xft-dpi"))
            g_value_set_int (value, screen_haiku->xft_settings.dpi);
          else
  ...
  settings = (GSettings *)g_hash_table_lookup (screen_haiku->settings, entry->schema);
  switch (entry->type)
    {
    case G_TYPE_STRING:
    ...
    case G_TYPE_INT:
      if (g_str_equal (entry->setting, "gtk-fontconfig-timestamp"))
        g_value_set_uint (value, screen_haiku->dbus_settings.fontconfig_timestamp);
      else
                                  ...
                                  GValue           *value)
{
  ...
  if (session && strstr (session, "GNOME-Classic"))
    settings = (GSettings *)g_hash_table_lookup (screen_haiku->settings, CLASSIC_WM_SETTINGS_SCHEMA);

  if (settings == NULL)
    settings = (GSettings *)g_hash_table_lookup (screen_haiku->settings, WM_SETTINGS_SCHEMA);
                      ...
                      GValue           *value)
{
  ...
  if (screen_haiku->settings_portal == NULL)
    {
      settings = (GSettings *)g_hash_table_lookup (screen_haiku->settings,
...
{
  GdkHaikuScreen *haiku_screen = GDK_HAIKU_SCREEN (screen);
  int testbit = 1 << (test - 1);

  g_value_set_boolean (value, (haiku_screen->shell_capabilities & testbit) == testbit);

  return TRUE;
}

static gboolean
gdk_haiku_screen_get_setting (GdkScreen   *screen,
                                const gchar *name,
                                GValue      *value)
{
  GdkHaikuScreen *haiku_screen = GDK_HAIKU_SCREEN (screen);
  ...
  if (haiku_screen->settings != NULL &&
      g_hash_table_size (haiku_screen->settings) == 0)
    return FALSE;
  ...
  if (strcmp (name, "gtk-fontconfig-timestamp") == 0)
    {
      g_value_set_uint (value, haiku_screen->dbus_settings.fontconfig_timestamp);
      return TRUE;
    }

  if (strcmp (name, "gtk-modules") == 0)
    {
      g_value_set_string (value, haiku_screen->dbus_settings.modules);
      return TRUE;
    }
...
typedef struct _GdkHaikuVisual	GdkHaikuVisual;
typedef struct _GdkHaikuVisualClass	GdkHaikuVisualClass;
...
GType _gdk_haiku_visual_get_type (void);

G_DEFINE_TYPE (GdkHaikuVisual, _gdk_haiku_visual, GDK_TYPE_VISUAL)

static void
_gdk_haiku_visual_class_init (GdkHaikuVisualClass *klass)
{
}

static void
_gdk_haiku_visual_init (GdkHaikuVisual *visual)
{
}

static gint
gdk_haiku_screen_visual_get_best_depth (GdkScreen *screen)
{
  return 32;
}

static GdkVisualType
gdk_haiku_screen_visual_get_best_type (GdkScreen *screen)
{
  return GDK_VISUAL_TRUE_COLOR;
}

static GdkVisual*
gdk_haiku_screen_visual_get_best (GdkScreen *screen)
{
  return GDK_HAIKU_SCREEN (screen)->visual;
}

static GdkVisual*
gdk_haiku_screen_visual_get_best_with_depth (GdkScreen *screen,
					       gint       depth)
{
  if (depth == 32)
    return GDK_HAIKU_SCREEN (screen)->visual;
...
static GdkVisual*
gdk_haiku_screen_visual_get_best_with_type (GdkScreen     *screen,
...
static GdkVisual*
gdk_haiku_screen_visual_get_best_with_both (GdkScreen     *screen,
					      gint           depth,
					      GdkVisualType  visual_type)
{
...
static void
gdk_haiku_screen_query_depths  (GdkScreen  *screen,
				  gint      **depths,
				  gint       *count)
{
...
static void
gdk_haiku_screen_query_visual_types (GdkScreen      *screen,
...
static GList *
gdk_haiku_screen_list_visuals (GdkScreen *screen)
{
  GList *list;
  GdkHaikuScreen *screen_haiku;

  g_return_val_if_fail (GDK_IS_SCREEN (screen), NULL);
  screen_haiku = GDK_HAIKU_SCREEN (screen);

  list = g_list_append (NULL, screen_haiku->visual);

  return list;
}

#define GDK_TYPE_HAIKU_VISUAL              (_gdk_haiku_visual_get_type ())
#define GDK_HAIKU_VISUAL(object)           (G_TYPE_CHECK_INSTANCE_CAST ((object), GDK_TYPE_HAIKU_VISUAL, GdkHaikuVisual))

/* Currently, the Wayland backend only ever uses ARGB8888.
 */
static GdkVisual *
gdk_haiku_visual_new (GdkScreen *screen)
{
  GdkVisual *visual;

  visual = g_object_new (GDK_TYPE_HAIKU_VISUAL, NULL);
...
dbus_properties_change_cb (GDBusProxy         *proxy,
                           ...
                           gpointer            user_data)
{
  GdkHaikuScreen *screen_haiku = user_data;
  GVariant *value;
  gint64 timestamp;
  ...
  if (value != NULL)
    {
      ...
      if (timestamp > 0 && timestamp <= G_MAXUINT)
        screen_haiku->dbus_settings.fontconfig_timestamp = (guint)timestamp;
      else if (timestamp > G_MAXUINT)
        g_warning ("Could not handle fontconfig update: timestamp out of bound");

      notify_setting (GDK_SCREEN (screen_haiku), "gtk-fontconfig-timestamp");
  ...
  if (value != NULL)
    {
      ...
      screen_haiku->dbus_settings.modules = g_variant_dup_string (value, NULL);

      notify_setting (GDK_SCREEN (screen_haiku), "gtk-modules");
                               ...
                               gpointer      user_data)
{
  ...
  screen_haiku->dbus_proxy = proxy;
  screen_haiku->dbus_setting_change_id =
    g_signal_connect (screen_haiku->dbus_proxy,
                      "g-properties-changed",
                      G_CALLBACK (dbus_properties_change_cb),
                      screen_haiku);

  value = g_dbus_proxy_get_cached_property (screen_haiku->dbus_proxy,
                                            "FontconfigTimestamp");
...
GdkScreen *
_gdk_haiku_screen_new (GdkDisplay *display)
{
  GdkScreen *screen;
  GdkHaikuScreen *screen_haiku;

  screen = g_object_new (GDK_TYPE_HAIKU_SCREEN, NULL);

  screen_haiku = GDK_HAIKU_SCREEN (screen);
  screen_haiku->display = display;
  screen_haiku->width = 0;
  screen_haiku->height = 0;

  screen_haiku->visual = gdk_haiku_visual_new (screen);

  screen_haiku->root_window =
    _gdk_haiku_screen_create_root_window (screen,
                                            screen_haiku->width,
                                            screen_haiku->height);
...
static void
_gdk_haiku_screen_class_init (GdkHaikuScreenClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GdkScreenClass *screen_class = GDK_SCREEN_CLASS (klass);

  object_class->dispose = gdk_haiku_screen_dispose;
  object_class->finalize = gdk_haiku_screen_finalize;

  screen_class->get_display = gdk_haiku_screen_get_display;
  screen_class->get_width = gdk_haiku_screen_get_width;
  screen_class->get_height = gdk_haiku_screen_get_height;
  screen_class->get_width_mm = gdk_haiku_screen_get_width_mm;
  screen_class->get_height_mm = gdk_haiku_screen_get_height_mm;
  screen_class->get_number = gdk_haiku_screen_get_number;
  screen_class->get_root_window = gdk_haiku_screen_get_root_window;
  screen_class->get_system_visual = gdk_haiku_screen_get_system_visual;
  screen_class->get_rgba_visual = gdk_haiku_screen_get_rgba_visual;
  screen_class->is_composited = gdk_haiku_screen_is_composited;
  screen_class->make_display_name = gdk_haiku_screen_make_display_name;
  screen_class->get_active_window = gdk_haiku_screen_get_active_window;
  screen_class->get_window_stack = gdk_haiku_screen_get_window_stack;
  screen_class->broadcast_client_message = gdk_haiku_screen_broadcast_client_message;
  screen_class->get_setting = gdk_haiku_screen_get_setting;
  screen_class->visual_get_best_depth = gdk_haiku_screen_visual_get_best_depth;
  screen_class->visual_get_best_type = gdk_haiku_screen_visual_get_best_type;
  screen_class->visual_get_best = gdk_haiku_screen_visual_get_best;
  screen_class->visual_get_best_with_depth = gdk_haiku_screen_visual_get_best_with_depth;
  screen_class->visual_get_best_with_type = gdk_haiku_screen_visual_get_best_with_type;
  screen_class->visual_get_best_with_both = gdk_haiku_screen_visual_get_best_with_both;
  screen_class->query_depths = gdk_haiku_screen_query_depths;
  screen_class->query_visual_types = gdk_haiku_screen_query_visual_types;
  screen_class->list_visuals = gdk_haiku_screen_list_visuals;
}

static void
_gdk_haiku_screen_init (GdkHaikuScreen *screen_haiku)
{
}

static void
update_screen_size (GdkHaikuScreen *screen_haiku)
{
  GdkHaikuDisplay *display_haiku = GDK_HAIKU_DISPLAY (screen_haiku->display);
  ...
  if (screen_haiku->width_mm != width_mm ||
      screen_haiku->height_mm != height_mm)
    {
      emit_changed = TRUE;
      screen_haiku->width_mm = width_mm;
      screen_haiku->height_mm = height_mm;
    }

  if (screen_haiku->width != width ||
      screen_haiku->height != height)
    {
      emit_changed = TRUE;
      screen_haiku->width = width;
      screen_haiku->height = height;
    }

  if (emit_changed)
    g_signal_emit_by_name (screen_haiku, "size-changed");
}
...
screen_has_xdg_output_support (GdkScreen *screen)
{
  ...
  return (display_haiku->xdg_output_manager != NULL);
}

static gboolean
monitor_has_xdg_output (GdkHaikuMonitor *monitor)
{
  return (monitor->xdg_output != NULL);
}

static gboolean
should_update_monitor (GdkHaikuMonitor *monitor)
{
...
static gboolean
should_expect_xdg_output_done (GdkHaikuMonitor *monitor)
{
  GdkDisplay *display = GDK_MONITOR (monitor)->display;
  GdkHaikuDisplay *display_haiku = GDK_HAIKU_DISPLAY (display);

  return (monitor_has_xdg_output (monitor) &&
          display_haiku->xdg_output_version < NO_XDG_OUTPUT_DONE_SINCE_VERSION);
}

static void
apply_monitor_change (GdkHaikuMonitor *monitor)
{
  GDK_NOTE (MISC,
  ...
  GdkDisplay *display = GDK_MONITOR (monitor)->display;
  GdkHaikuScreen *screen_haiku = GDK_HAIKU_SCREEN (gdk_display_get_default_screen (display));
  ...
  g_signal_emit_by_name (screen_haiku, "monitors-changed");
  update_screen_size (screen_haiku);
}
...
xdg_output_handle_logical_position (void                  *data,
                                    ...
                                    int32_t                y)
{
  GdkHaikuMonitor *monitor = (GdkHaikuMonitor *) data;
...
static void
gdk_haiku_screen_get_xdg_output (GdkHaikuMonitor *monitor)
{
  GdkDisplay *display = GDK_MONITOR (monitor)->display;
  GdkHaikuDisplay *display_haiku = GDK_HAIKU_DISPLAY (display);
  ...
  monitor->xdg_output =
    zxdg_output_manager_v1_get_xdg_output (display_haiku->xdg_output_manager,
                                           monitor->output);
...
void
_gdk_haiku_screen_add_output (GdkScreen        *screen,
                                guint32           id,
                                struct wl_output *output,
				guint32           version)
{
  GdkDisplay *display = gdk_screen_get_display (screen);
  GdkHaikuMonitor *monitor;
  ...
  g_ptr_array_add (GDK_HAIKU_DISPLAY (display)->monitors, monitor);
  gdk_display_monitor_added (display, GDK_MONITOR (monitor));
  wl_output_add_listener (output, &output_listener, monitor);

  GDK_NOTE (MISC,
            g_message ("xdg_output_manager %p",
                       GDK_HAIKU_DISPLAY (display)->xdg_output_manager));

  if (screen_has_xdg_output_support (screen))
    gdk_haiku_screen_get_xdg_output (monitor);
}

struct wl_output *
_gdk_haiku_screen_get_wl_output (GdkScreen *screen,
                                   gint monitor_num)
{
  GdkHaikuDisplay *display_haiku = GDK_HAIKU_DISPLAY (GDK_HAIKU_SCREEN (screen)->display);
  GdkHaikuMonitor *monitor;

  monitor = display_haiku->monitors->pdata[monitor_num];

  return monitor->output;
}

static GdkHaikuMonitor *
get_monitor_for_id (GdkHaikuScreen *screen_haiku,
                    guint32           id)
{
  GdkHaikuDisplay *display_haiku = GDK_HAIKU_DISPLAY (screen_haiku->display);
  int i;
  ...
  for (i = 0; i < display_haiku->monitors->len; i++)
    {
      GdkHaikuMonitor *monitor = display_haiku->monitors->pdata[i];
...
static GdkHaikuMonitor *
get_monitor_for_output (GdkHaikuScreen *screen_haiku,
                        struct wl_output *output)
{
  GdkHaikuDisplay *display_haiku = GDK_HAIKU_DISPLAY (screen_haiku->display);
  int i;
...
void
_gdk_haiku_screen_remove_output (GdkScreen *screen,
                                   guint32    id)
{
  GdkHaikuScreen *screen_haiku = GDK_HAIKU_SCREEN (screen);
  GdkHaikuDisplay *display_haiku = GDK_HAIKU_DISPLAY (screen_haiku->display);
  ...
  monitor = get_monitor_for_id (screen_haiku, id);
  if (monitor != NULL)
    {
      g_object_ref (monitor);
      g_ptr_array_remove (display_haiku->monitors, monitor);
      gdk_display_monitor_removed (GDK_DISPLAY (display_haiku), GDK_MONITOR (monitor));
...
int
_gdk_haiku_screen_get_output_refresh_rate (GdkScreen        *screen,
                                             struct wl_output *output)
{
  GdkHaikuScreen *screen_haiku = GDK_HAIKU_SCREEN (screen);
  GdkHaikuMonitor *monitor;

  monitor = get_monitor_for_output (screen_haiku, output);
  if (monitor != NULL)
...
guint32
_gdk_haiku_screen_get_output_scale (GdkScreen        *screen,
				      struct wl_output *output)
{
  GdkHaikuScreen *screen_haiku = GDK_HAIKU_SCREEN (screen);
  GdkHaikuMonitor *monitor;
...
void
_gdk_haiku_screen_init_xdg_output (GdkScreen *screen)
{
  GdkDisplay *display = gdk_screen_get_display (screen);
  GdkHaikuDisplay *display_haiku = GDK_HAIKU_DISPLAY (display);
  int i;
  ...
  GDK_NOTE (MISC,
            g_message ("init xdg-output support, %d monitor(s) already present",
                       display_haiku->monitors->len));

  for (i = 0; i < display_haiku->monitors->len; i++)
    gdk_haiku_screen_get_xdg_output (display_haiku->monitors->pdata[i]);
}
