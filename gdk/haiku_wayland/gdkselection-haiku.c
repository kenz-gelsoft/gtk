...
static void emit_empty_selection_notify (GdkWindow *requestor,
                                         GdkAtom    selection,
                                         GdkAtom    target);
static void gdk_haiku_selection_handle_next_request (GdkHaikuSelection *haiku_selection);
...
stored_selection_new (GdkHaikuSelection *haiku_selection,
                      GdkWindow           *source,
                      GdkAtom              selection,
                      GdkAtom              type)
{
  ...
  stored_selection->selection = haiku_selection;
...
gdk_haiku_selection_new (void)
{
  ...
  atoms[ATOM_DND] = gdk_atom_intern_static_string ("GdkHaikuSelection");

  selection = g_new0 (GdkHaikuSelection, 1);
...
void
gdk_haiku_selection_free (GdkHaikuSelection *selection)
{
  gint i;
...
data_offer_offer (void                 *data,
                  ...
                  const char           *type)
{
  GdkHaikuSelection *selection = data;
  DataOfferData *info;
                           ...
                           uint32_t              source_actions)
{
  ...
  drop_context = gdk_haiku_device_get_drop_context (device);
  ...
  if (gdk_drag_context_get_dest_window (drop_context))
    _gdk_haiku_drag_context_emit_event (drop_context, GDK_DRAG_MOTION,
                                          GDK_CURRENT_TIME);
}
...
SelectionData *
selection_lookup_offer_by_atom (GdkHaikuSelection *selection,
                                GdkAtom              selection_atom)
{
...
void
gdk_haiku_selection_ensure_offer (GdkDisplay           *display,
                                    struct wl_data_offer *wl_offer)
{
  GdkHaikuSelection *selection = gdk_haiku_display_get_selection (display);
  DataOfferData *info;
...
void
gdk_haiku_selection_ensure_primary_offer (GdkDisplay *display,
                                            gpointer    gtk_offer)
{
  GdkHaikuDisplay *display_haiku = GDK_HAIKU_DISPLAY (display);
  GdkHaikuSelection *selection = gdk_haiku_display_get_selection (display);
...
void
gdk_haiku_selection_set_offer (GdkDisplay *display,
                                 GdkAtom     selection_atom,
                                 gpointer    wl_offer)
{
  GdkHaikuSelection *selection = gdk_haiku_display_get_selection (display);
  ...
  prev_offer = gdk_haiku_selection_get_offer (display, selection_atom);
...
gpointer
gdk_haiku_selection_get_offer (GdkDisplay *display,
                                 GdkAtom     selection_atom)
{
  GdkHaikuSelection *selection = gdk_haiku_display_get_selection (display);
...
GList *
gdk_haiku_selection_get_targets (GdkDisplay *display,
                                   GdkAtom     selection_atom)
{
  GdkHaikuSelection *selection = gdk_haiku_display_get_selection (display);
  const SelectionData *data;
...
static void
gdk_haiku_selection_emit_request (GdkWindow *window,
                                    GdkAtom    selection,
                                    GdkAtom    target)
{
  GdkEvent *event;
...
static StoredSelection *
gdk_haiku_selection_find_stored_selection (GdkHaikuSelection *haiku_selection,
                                             GdkWindow           *window,
                                             GdkAtom              selection,
                                             GdkAtom              type)
{
  ...
  for (i = 0; i < haiku_selection->stored_selections->len; i++)
    {
      StoredSelection *stored_selection;

      stored_selection = g_ptr_array_index (haiku_selection->stored_selections, i);
...
static void
gdk_haiku_selection_reset_selection (GdkHaikuSelection *haiku_selection,
                                       GdkAtom              selection)
{
  gint i = 0;

  while (i < haiku_selection->stored_selections->len)
    {
      StoredSelection *stored_selection;
      ...
      if (stored_selection->selection_atom == selection)
        {
          if (haiku_selection->current_request_selection == stored_selection)
            haiku_selection->current_request_selection = NULL;

          g_ptr_array_remove_index_fast (haiku_selection->stored_selections, i);
        }
      else
        i++;
    }
...
void
gdk_haiku_selection_store (GdkWindow    *window,
                             GdkAtom       type,
                             GdkPropMode   mode,
                             const guchar *data,
                             gint          len)
{
  ...
  gdk_haiku_selection_handle_next_request (selection);
}

static SelectionBuffer *
gdk_haiku_selection_lookup_requestor_buffer (GdkWindow *requestor)
{
...
static gboolean
gdk_haiku_selection_source_handles_target (GdkHaikuSelection *haiku_selection,
                                             GdkAtom             *target)
{
  GdkAtom atom;
  ...
  for (i = 0; i < haiku_selection->source_targets->len; i++)
    {
      atom = g_array_index (haiku_selection->source_targets, GdkAtom, i);
...
static void
gdk_haiku_selection_handle_next_request (GdkHaikuSelection *haiku_selection)
{
  gint i;
  ...
  for (i = 0; i < haiku_selection->stored_selections->len; i++)
    {
      ...
      if (!stored_selection->data)
        {
          gdk_haiku_selection_emit_request (stored_selection->source,
                                              stored_selection->selection_atom,
                                              stored_selection->type);
          haiku_selection->current_request_selection = stored_selection;
          break;
        }
    }
}
...
static gboolean
gdk_haiku_selection_request_target (GdkHaikuSelection *haiku_selection,
                                      GdkWindow           *window,
                                      GdkAtom              selection,
                                      GdkAtom              target,
                                      gint                 fd)
{
  ...
  if (!window ||
      !gdk_haiku_selection_source_handles_target (haiku_selection, &target))
    {
      close (fd);
      return FALSE;
    }
  ...
  stored_selection =
    gdk_haiku_selection_find_stored_selection (haiku_selection, window,
                                                 selection, target);
  ...
  if (!stored_selection)
    {
      stored_selection = stored_selection_new (haiku_selection, window,
                                               selection, target);
      g_ptr_array_add (haiku_selection->stored_selections, stored_selection);
    }

  write_data = async_write_data_new (stored_selection, fd);

  if (!haiku_selection->current_request_selection)
    gdk_haiku_selection_handle_next_request (haiku_selection);
...
data_source_send (void                  *data,
                  ...
                  int32_t                fd)
{
  GdkHaikuSelection *haiku_selection = data;
  GdkWindow *window;
  GdkAtom selection;
  ...
  if (source == haiku_selection->dnd_source)
    {
      window = haiku_selection->dnd_owner;
      selection = atoms[ATOM_DND];
    }
  else if (source == haiku_selection->clipboard_source)
    {
      window = haiku_selection->clipboard_owner;
  ...
  gdk_haiku_selection_request_target (haiku_selection, window,
                                        selection,
                       ...
                       struct wl_data_source *source)
{
  ...
  context = gdk_haiku_drag_context_lookup_by_data_source (source);
  ...
  gdk_haiku_selection_unset_data_source (display, atom);
}
                          ...
                          struct wl_data_source *source)
{
  ...
  if (context->action == GDK_ACTION_MOVE)
    {
      gdk_haiku_selection_emit_request (context->source_window,
                                          atoms[ATOM_DND],
  ...
  gdk_haiku_selection_clear_targets (display, atoms[ATOM_DND]);
}
                     ...
                     int32_t     fd)
{
  ...
  if (!mime_type || !haiku_selection->primary_owner)
    {
      close (fd);
      return;
    }

  gdk_haiku_selection_request_target (haiku_selection,
                                        haiku_selection->primary_owner,
                                        atoms[ATOM_PRIMARY],
                                        gdk_atom_intern (mime_type, FALSE),
                                        fd);
}
...
gdk_haiku_selection_get_data_source (GdkWindow *owner,
                                       GdkAtom    selection)
{
  GdkDisplay *display = gdk_window_get_display (owner);
  GdkHaikuSelection *haiku_selection = gdk_haiku_display_get_selection (display);
  gpointer source = NULL;
  GdkHaikuDisplay *display_haiku;

  if (selection == atoms[ATOM_DND])
    {
      if (haiku_selection->dnd_source &&
          (!owner || owner == haiku_selection->dnd_owner))
        return haiku_selection->dnd_source;
    }
  else if (selection == atoms[ATOM_PRIMARY])
    {
      if (haiku_selection->primary_source &&
          (!owner || owner == haiku_selection->primary_owner))
        return haiku_selection->primary_source;

      if (haiku_selection->primary_source)
        {
          primary_selection_source_destroy (haiku_selection->primary_source);
          haiku_selection->primary_source = NULL;
        }
    }
  else if (selection == atoms[ATOM_CLIPBOARD])
    {
      if (haiku_selection->clipboard_source &&
          (!owner || owner == haiku_selection->clipboard_owner))
        return haiku_selection->clipboard_source;

      if (haiku_selection->clipboard_source)
        {
          wl_data_source_destroy (haiku_selection->clipboard_source);
          haiku_selection->clipboard_source = NULL;
        }
    }
  else
    return NULL;
  ...
  display_haiku = GDK_HAIKU_DISPLAY (gdk_window_get_display (owner));

  if (selection == atoms[ATOM_PRIMARY])
    {
      if (display_haiku->zwp_primary_selection_manager_v1)
        {
          source = zwp_primary_selection_device_manager_v1_create_source (display_haiku->zwp_primary_selection_manager_v1);
        ...
        {
          source = gtk_primary_selection_device_manager_create_source (display_haiku->gtk_primary_selection_manager);
    ...
    {
      source = wl_data_device_manager_create_data_source (display_haiku->data_device_manager);
  ...
  if (selection == atoms[ATOM_DND])
    haiku_selection->dnd_source = source;
  else if (selection == atoms[ATOM_PRIMARY])
    haiku_selection->primary_source = source;
  else if (selection == atoms[ATOM_CLIPBOARD])
    haiku_selection->clipboard_source = source;

  return source;
}

void
gdk_haiku_selection_unset_data_source (GdkDisplay *display,
                                         GdkAtom     selection)
{
  GdkHaikuSelection *haiku_selection = gdk_haiku_display_get_selection (display);
  ...
  if (selection == atoms[ATOM_CLIPBOARD])
    ...
    {
      haiku_selection->dnd_source = NULL;
    }
}
                                          ...
                                          GdkAtom     selection)
{
  ...
  if (selection == atoms[ATOM_CLIPBOARD])
    return haiku_selection->clipboard_owner;
  else if (selection == atoms[ATOM_PRIMARY])
    return haiku_selection->primary_owner;
  else if (selection == atoms[ATOM_DND])
    return haiku_selection->dnd_owner;
                                          ...
                                          gboolean    send_event)
{
  ...
  gdk_haiku_selection_reset_selection (haiku_selection, selection);

  if (selection == atoms[ATOM_CLIPBOARD])
    {
      haiku_selection->clipboard_owner = owner;
      if (send_event && !owner)
        {
          gdk_haiku_seat_set_selection (seat, NULL);
    ...
    {
      haiku_selection->primary_owner = owner;
      if (send_event && !owner)
        {
          gdk_haiku_seat_set_primary (seat, NULL);
          gdk_haiku_selection_unset_data_source (display, selection);
        }
      return TRUE;
    }
    ...
    {
      haiku_selection->dnd_owner = owner;
      return TRUE;
    }
...
void
_gdk_haiku_display_send_selection_notify (GdkDisplay *display,
                                            GdkWindow  *requestor,
                                            GdkAtom     selection,
                                            GdkAtom     target,
                                            GdkAtom     property,
                                            guint32     time)
{
  GdkHaikuSelection *haiku_selection;

  if (property != GDK_NONE)
    return;

  haiku_selection = gdk_haiku_display_get_selection (display);

  if (!haiku_selection->current_request_selection)
    return;

  g_ptr_array_remove_fast (haiku_selection->stored_selections,
                           haiku_selection->current_request_selection);
                                             ...
                                             gint        *ret_format)
{
  ...
  buffer_data = gdk_haiku_selection_lookup_requestor_buffer (requestor);
                      ...
                      GdkAtom     selection)
{
  ...
  window = _gdk_haiku_display_get_selection_owner (display, selection);
  if (window != NULL)
    {
      event->selection.window = g_object_ref (window);
      event->selection.requestor = g_object_ref (window);
                                        ...
                                        guint32     time)
{
  ...
  selection_data = selection_lookup_offer_by_atom (haiku_selection, selection);
  if (!selection_data)
    {
  ...
  offer = gdk_haiku_selection_get_offer (display, selection);
  target_list = gdk_haiku_selection_get_targets (display, selection);
  ...
  if (target != gdk_atom_intern_static_string ("TARGETS"))
    {
      ...
      if (selection != atoms[ATOM_PRIMARY])
        wl_data_offer_accept (offer,
                              _gdk_haiku_display_get_serial (GDK_HAIKU_DISPLAY (display)),
                              mimetype);
    }
                                            ...
                                            const gchar *str)
{
   ...
   * Xhaiku, the STRING target could be used, and passed as-is
   * by the compositor.
   *
...
void
gdk_haiku_selection_add_targets (GdkWindow *window,
                                   GdkAtom    selection,
                                   guint      ntargets,
                                   GdkAtom   *targets)
{
  ...
  data_source = gdk_haiku_selection_get_data_source (window, selection);

  if (!data_source)
    return;

  g_array_append_vals (haiku_selection->source_targets, targets, ntargets);
  ...
  if (selection == atoms[ATOM_CLIPBOARD])
    {
      ...
      seat = gdk_display_get_default_seat (display);
      gdk_haiku_seat_set_selection (seat, data_source);
    }
  else if (selection == atoms[ATOM_PRIMARY])
    {
      GdkSeat *seat;
      ...
      seat = gdk_display_get_default_seat (display);
      gdk_haiku_seat_set_primary (seat, data_source);
    }
}

void
gdk_haiku_selection_clear_targets (GdkDisplay *display,
                                     GdkAtom     selection)
{
  GdkHaikuSelection *haiku_selection = gdk_haiku_display_get_selection (display);

  haiku_selection->requested_target = GDK_NONE;
  g_array_set_size (haiku_selection->source_targets, 0);
  gdk_haiku_selection_unset_data_source (display, selection);
}

gboolean
gdk_haiku_selection_set_current_offer_actions (GdkDisplay *display,
                                                 uint32_t    action)
{
  GdkHaikuDisplay *display_haiku = GDK_HAIKU_DISPLAY (display);
  ...
  offer = gdk_haiku_selection_get_offer (display, atoms[ATOM_DND]);
