...
#define GDK_TYPE_HAIKU_KEYMAP          (_gdk_haiku_keymap_get_type ())
...
GType _gdk_haiku_keymap_get_type (void);
...
{
  struct xkb_keymap *xkb_keymap = GDK_HAIKU_KEYMAP (keymap)->xkb_keymap;
  struct xkb_state *xkb_state = GDK_HAIKU_KEYMAP (keymap)->xkb_state;
  xkb_mod_mask_t mods;
...
static void
update_direction (GdkHaikuKeymap *keymap)
{
  gint num_layouts;
  gint i;
  gint *rtl;
...
GdkKeymap *
_gdk_haiku_keymap_new (void)
{
  GdkHaikuKeymap *keymap;
  struct xkb_context *context;
  struct xkb_rule_names names;

  keymap = g_object_new (_gdk_haiku_keymap_get_type(), NULL);
...
void
_gdk_haiku_keymap_update_from_fd (GdkKeymap *keymap,
                                    uint32_t   format,
                                    uint32_t   fd,
                                    uint32_t   size)
{
  ...
  xkb_keymap_unref (keymap_haiku->xkb_keymap);
  keymap_haiku->xkb_keymap = xkb_keymap;

  xkb_state_unref (keymap_haiku->xkb_state);
  keymap_haiku->xkb_state = xkb_state_new (keymap_haiku->xkb_keymap);

  xkb_context_unref (context);

  update_direction (keymap_haiku);
}

struct xkb_keymap *
_gdk_haiku_keymap_get_xkb_keymap (GdkKeymap *keymap)
{
  return GDK_HAIKU_KEYMAP (keymap)->xkb_keymap;
}

struct xkb_state *
_gdk_haiku_keymap_get_xkb_state (GdkKeymap *keymap)
{
