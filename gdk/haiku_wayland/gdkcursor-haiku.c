...
#include <haiku-cursor.h>

#define GDK_TYPE_HAIKU_CURSOR              (_gdk_haiku_cursor_get_type ())
...
GType _gdk_haiku_cursor_get_type (void);

G_DEFINE_TYPE (GdkHaikuCursor, _gdk_haiku_cursor, GDK_TYPE_CURSOR)

void
_gdk_haiku_display_init_cursors (GdkHaikuDisplay *display)
{
  display->cursor_cache = g_hash_table_new_full (g_str_hash, g_str_equal, NULL, g_object_unref);
}

void
_gdk_haiku_display_finalize_cursors (GdkHaikuDisplay *display)
{
...
static gboolean
_gdk_haiku_cursor_update (GdkHaikuDisplay *display_haiku,
                            GdkHaikuCursor  *cursor)
{
  struct wl_cursor *c;
  ...
  theme = _gdk_haiku_display_get_scaled_cursor_theme (display_haiku,
                                                        cursor->scale);
...
void
_gdk_haiku_display_update_cursors (GdkHaikuDisplay *display)
{
  GHashTableIter iter;
  const char *name;
  GdkHaikuCursor *cursor;
...
const char *
_gdk_haiku_cursor_get_name (GdkCursor *cursor)
{
  GdkHaikuCursor *haiku_cursor = GDK_HAIKU_CURSOR (cursor);
  return haiku_cursor->name;
}

struct wl_buffer *
_gdk_haiku_cursor_get_buffer (GdkCursor *cursor,
                                guint      image_index,
                                int       *hotspot_x,
                                int       *hotspot_y,
                                int       *w,
                                ...
                                int       *scale)
{
  ...
  if (haiku_cursor->wl_cursor && haiku_cursor->wl_cursor->image_count > 0)
    {
      struct wl_cursor_image *image;
      int cursor_scale;

      if (image_index >= haiku_cursor->wl_cursor->image_count)
        {
          g_warning (G_STRLOC " out of bounds cursor image [%d / %d]",
                     image_index,
                     haiku_cursor->wl_cursor->image_count - 1);
          image_index = 0;
        }

      image = haiku_cursor->wl_cursor->images[image_index];

      cursor_scale = haiku_cursor->scale;
  ...
  else if (haiku_cursor->name == NULL) /* From surface */
    {
      *hotspot_x =
        haiku_cursor->surface.hotspot_x / haiku_cursor->surface.scale;
      *hotspot_y =
        haiku_cursor->surface.hotspot_y / haiku_cursor->surface.scale;

      *w = haiku_cursor->surface.width / haiku_cursor->surface.scale;
      *h = haiku_cursor->surface.height / haiku_cursor->surface.scale;
      *scale = haiku_cursor->surface.scale;

      cairo_surface_reference (haiku_cursor->surface.cairo_surface);

      if (haiku_cursor->surface.cairo_surface)
        return _gdk_haiku_shm_surface_get_wl_buffer (haiku_cursor->surface.cairo_surface);
    }
  else
    {
      *hotspot_x = 0;
...
guint
_gdk_haiku_cursor_get_next_image_index (GdkCursor *cursor,
                                          guint      current_image_index,
                                          guint     *next_image_delay)
{
  struct wl_cursor *wl_cursor = GDK_HAIKU_CURSOR (cursor)->wl_cursor;
...
void
_gdk_haiku_cursor_set_scale (GdkCursor *cursor,
                               guint      scale)
{
  GdkHaikuDisplay *display_haiku =
    GDK_HAIKU_DISPLAY (gdk_cursor_get_display (cursor));
  GdkHaikuCursor *haiku_cursor = GDK_HAIKU_CURSOR (cursor);

  if (scale > GDK_HAIKU_MAX_THEME_SCALE)
    {
      g_warning (G_STRLOC ": cursor theme size %u too large", scale);
      scale = GDK_HAIKU_MAX_THEME_SCALE;
    }

  if (haiku_cursor->scale == scale)
    return;

  haiku_cursor->scale = scale;

  /* Blank cursor case */
  if (g_strcmp0 (haiku_cursor->name, "none") == 0)
    return;
...
_gdk_haiku_display_get_cursor_for_name_with_scale (GdkDisplay  *display,
                                                     ...
                                                     guint        scale)
{
  GdkHaikuCursor *haiku_cursor;
  GdkHaikuDisplay *display_haiku = GDK_HAIKU_DISPLAY (display);

  g_return_val_if_fail (GDK_IS_DISPLAY (display), NULL);

  haiku_cursor = g_hash_table_lookup (display_haiku->cursor_cache, name);
  if (haiku_cursor && haiku_cursor->scale == scale)
    return GDK_CURSOR (g_object_ref (haiku_cursor));

  haiku_cursor = g_object_new (GDK_TYPE_HAIKU_CURSOR,
  ...
  if (!name || g_str_equal (name, "none") || g_str_equal (name, "blank_cursor"))
    {
      haiku_cursor->name = g_strdup ("none");
      haiku_cursor->scale = scale;

      return GDK_CURSOR (haiku_cursor);
    }

  haiku_cursor->name = g_strdup (name);
  haiku_cursor->scale = scale;

  if (!_gdk_haiku_cursor_update (display_haiku, haiku_cursor))
    {
      g_object_unref (haiku_cursor);
      return NULL;
    }

  /* Insert into cache. */
  g_hash_table_replace (display_haiku->cursor_cache,
                        haiku_cursor->name,
...
GdkCursor *
_gdk_haiku_display_get_cursor_for_type_with_scale (GdkDisplay    *display,
                                                     ...
                                                     guint          scale)
{
  ...
  result = _gdk_haiku_display_get_cursor_for_name_with_scale (display,
                                                                cursor_name,
                                                                scale);
...
_gdk_haiku_display_get_cursor_for_type (GdkDisplay    *display,
                                          GdkCursorType  cursor_type)
{
  return _gdk_haiku_display_get_cursor_for_type_with_scale (display,
                                                              cursor_type,
                                                              1);
}
					     ...
					     gdouble     y)
{
  ...
  cursor = g_object_new (GDK_TYPE_HAIKU_CURSOR,
			 "cursor-type", GDK_CURSOR_IS_PIXMAP,
			 "display", display_haiku,
			 NULL);
  cursor->name = NULL;
  ...
  cursor->surface.cairo_surface =
    _gdk_haiku_display_create_shm_surface (display_haiku,
                                             cursor->surface.width / cursor->surface.scale,
                                             cursor->surface.height / cursor->surface.scale,
                                             cursor->surface.scale);

  buffer = _gdk_haiku_shm_surface_get_wl_buffer (cursor->surface.cairo_surface);
