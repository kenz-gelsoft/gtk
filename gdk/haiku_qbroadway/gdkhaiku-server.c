...
static GType gdk_haiku_server_get_type (void);

G_DEFINE_TYPE (GdkHaikuServer, gdk_haiku_server, G_TYPE_OBJECT)

static void
gdk_haiku_server_init (GdkHaikuServer *server)
{
  server->next_serial = 1;
}

static void
gdk_haiku_server_finalize (GObject *object)
{
  G_OBJECT_CLASS (gdk_haiku_server_parent_class)->finalize (object);
}

static void
gdk_haiku_server_class_init (GdkHaikuServerClass * class)
{
  GObjectClass *object_class = G_OBJECT_CLASS (class);

  object_class->finalize = gdk_haiku_server_finalize;
}

gboolean
_gdk_haiku_server_lookahead_event (GdkHaikuServer  *server,
				      const char         *types)
{
...
gulong
_gdk_haiku_server_get_next_serial (GdkHaikuServer *server)
{
  return (gulong)server->next_serial;
}

GdkHaikuServer *
_gdk_haiku_server_new (const char *display, GError **error)
{
  GdkHaikuServer *server;
  ...
  if (g_str_has_prefix (display, ":tcp"))
  ...
  else
    {
      g_set_error (error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
		   _("Haiku display type not supported: %s"), display);
      return NULL;
    }
  ...
  server = g_object_new (GDK_TYPE_HAIKU_SERVER, NULL);
...
guint32
_gdk_haiku_server_get_last_seen_time (GdkHaikuServer *server)
{
  return 0;
}

static guint32
gdk_haiku_server_send_message_with_size (GdkHaikuServer *server, HaikuRequestBase *base,
...
#define gdk_haiku_server_send_message(_server, _msg, _type) \
  gdk_haiku_server_send_message_with_size(_server, (HaikuRequestBase *)&_msg, sizeof (_msg), _type)

static void
parse_all_input (GdkHaikuServer *server)
{
  guint8 *p, *end;
  guint32 size;
  HaikuReply *reply;
...
static void
read_some_input_blocking (GdkHaikuServer *server)
{
  GInputStream *in;
  gssize res;
  ...
  if (res <= 0)
    {
      g_printerr ("Unable to read from haiku server\n");
      exit (1);
    }
...
static void
read_some_input_nonblocking (GdkHaikuServer *server)
{
  GInputStream *in;
  GPollableInputStream *pollable;
  gssize res;
  ...
  if (res < 0 && g_error_matches (error, G_IO_ERROR, G_IO_ERROR_WOULD_BLOCK))
    ...
    {
      g_printerr ("Unable to read from haiku server: %s\n", error ? error->message : "eof");
      exit (1);
    }
...
static HaikuReply *
find_response_by_serial (GdkHaikuServer *server, guint32 serial)
{
  GList *l;

  for (l = server->incomming; l != NULL; l = l->next)
    {
      HaikuReply *reply = l->data;
...
static void
process_input_messages (GdkHaikuServer *server)
{
  HaikuReply *reply;
  ...
  while (server->incomming)
	...
	g_list_delete_link (server->incomming,
      ...
      if (reply->base.type == HAIKU_REPLY_EVENT)
	_gdk_haiku_events_got_input (&reply->event.msg);
      else
...
static gboolean
process_input_idle_cb (GdkHaikuServer *server)
{
  server->process_input_idle = 0;
...
static void
queue_process_input_at_idle (GdkHaikuServer *server)
{
  if (server->process_input_idle == 0)
...
{
  GdkHaikuServer *server = user_data;
...
static HaikuReply *
gdk_haiku_server_wait_for_reply (GdkHaikuServer *server,
				    guint32 serial)
{
  HaikuReply *reply;
...
void
_gdk_haiku_server_flush (GdkHaikuServer *server)
{
  HaikuRequestFlush msg;

  gdk_haiku_server_send_message(server, msg, HAIKU_REQUEST_FLUSH);
}

void
_gdk_haiku_server_sync (GdkHaikuServer *server)
{
  HaikuRequestSync msg;
  guint32 serial;
  HaikuReply *reply;

  serial = gdk_haiku_server_send_message (server, msg,
					     HAIKU_REQUEST_SYNC);
  reply = gdk_haiku_server_wait_for_reply (server, serial);

  g_assert (reply->base.type == HAIKU_REPLY_SYNC);
...
void
_gdk_haiku_server_query_mouse (GdkHaikuServer *server,
				  guint32            *toplevel,
				  gint32             *root_x,
				  gint32             *root_y,
				  guint32            *mask)
{
  HaikuRequestQueryMouse msg;
  guint32 serial;
  HaikuReply *reply;
  ...
  g_assert (reply->base.type == HAIKU_REPLY_QUERY_MOUSE);
...
guint32
_gdk_haiku_server_new_window (GdkHaikuServer *server,
				 int x,
				 int y,
				 int width,
				 int height,
				 gboolean is_temp)
{
  ...
  g_assert (reply->base.type == HAIKU_REPLY_NEW_WINDOW);
...
void
_gdk_haiku_server_destroy_window (GdkHaikuServer *server,
				     gint id)
{
  HaikuRequestDestroyWindow msg;

  msg.id = id;
  gdk_haiku_server_send_message (server, msg,
				    HAIKU_REQUEST_DESTROY_WINDOW);
}

gboolean
_gdk_haiku_server_window_show (GdkHaikuServer *server,
				  gint id)
{
  HaikuRequestShowWindow msg;
...
gboolean
_gdk_haiku_server_window_hide (GdkHaikuServer *server,
				  gint id)
{
  HaikuRequestHideWindow msg;
...
void
_gdk_haiku_server_window_focus (GdkHaikuServer *server,
				   gint id)
{
  HaikuRequestFocusWindow msg;
...
void
_gdk_haiku_server_window_set_transient_for (GdkHaikuServer *server,
					       gint id, gint parent)
{
  HaikuRequestSetTransientFor msg;
...
void
_gdk_haiku_server_window_set_modal_hint (GdkHaikuServer *server,
					    gint id, gboolean modal_hint)
{
  HaikuRequestSetModalHint msg;
...
static const cairo_user_data_key_t gdk_haiku_shm_cairo_key;
...
{
  HaikuShmSurfaceData *data = _data;
...
cairo_surface_t *
_gdk_haiku_server_create_surface (int                 width,
				     int                 height)
{
  HaikuShmSurfaceData *data;
  cairo_surface_t *surface;

  data = g_new (HaikuShmSurfaceData, 1);
  ...
  cairo_surface_set_user_data (surface, &gdk_haiku_shm_cairo_key,
...
void
_gdk_haiku_server_window_update (GdkHaikuServer *server,
				    gint id,
				    cairo_surface_t *surface)
{
  HaikuRequestUpdate msg;
  HaikuShmSurfaceData *data;

  if (surface == NULL)
    return;

  data = cairo_surface_get_user_data (surface, &gdk_haiku_shm_cairo_key);
  g_assert (data != NULL);
...
gboolean
_gdk_haiku_server_window_move_resize (GdkHaikuServer *server,
					 gint id,
					 gboolean with_move,
					 int x,
					 int y,
...
GdkGrabStatus
_gdk_haiku_server_grab_pointer (GdkHaikuServer *server,
				   gint id,
				   gboolean owner_events,
				   guint32 event_mask,
				   guint32 time_)
{
  HaikuRequestGrabPointer msg;
  ...
  g_assert (reply->base.type == HAIKU_REPLY_GRAB_POINTER);
...
guint32
_gdk_haiku_server_ungrab_pointer (GdkHaikuServer *server,
				     guint32    time_)
{
  HaikuRequestUngrabPointer msg;
  guint32 serial, status;
  HaikuReply *reply;
  ...
  g_assert (reply->base.type == HAIKU_REPLY_UNGRAB_POINTER);
...
void
_gdk_haiku_server_set_show_keyboard (GdkHaikuServer *server,
                                        gboolean show)
{
  HaikuRequestSetShowKeyboard msg;
