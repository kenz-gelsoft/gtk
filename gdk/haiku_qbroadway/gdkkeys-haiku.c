...
typedef struct _GdkKeymapClass GdkHaikuKeymapClass;
...
static GType gdk_haiku_keymap_get_type (void);
...
G_DEFINE_TYPE (GdkHaikuKeymap, gdk_haiku_keymap, GDK_TYPE_KEYMAP)

static void  gdk_haiku_keymap_finalize   (GObject           *object);
...
_gdk_haiku_display_get_keymap (GdkDisplay *display)
{
  ...
  if (!haiku_display->keymap)
    haiku_display->keymap = g_object_new (gdk_haiku_keymap_get_type (), NULL);

  haiku_display->keymap->display = display;

  return haiku_display->keymap;
}
...
static void
gdk_haiku_keymap_class_init (GdkHaikuKeymapClass *klass)
{
