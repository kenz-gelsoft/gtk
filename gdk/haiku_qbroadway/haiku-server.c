...
#include "haiku-server.h"

#include "haiku-output.h"
...
typedef struct HaikuInput HaikuInput;
typedef struct HaikuWindow HaikuWindow;
struct _HaikuServer {
...
static void haiku_server_resync_windows (HaikuServer *server);

static GType haiku_server_get_type (void);

G_DEFINE_TYPE (HaikuServer, haiku_server, G_TYPE_OBJECT)

static void
haiku_server_init (HaikuServer *server)
{
  HaikuWindow *root;
...
static void
haiku_server_finalize (GObject *object)
{
  HaikuServer *server = HAIKU_SERVER (object);

  g_free (server->address);
  g_free (server->ssl_cert);
  g_free (server->ssl_key);

  G_OBJECT_CLASS (haiku_server_parent_class)->finalize (object);
}

static void
haiku_server_class_init (HaikuServerClass * class)
{
  GObjectClass *object_class = G_OBJECT_CLASS (class);

  object_class->finalize = haiku_server_finalize;
}

static void start (HaikuInput *input);
...
static void
haiku_input_free (HaikuInput *input)
{
...
static void
update_event_state (HaikuServer *server,
		    HaikuInputMsg *message)
{
  HaikuWindow *window;

  switch (message->base.type) {
  case HAIKU_EVENT_ENTER:
  ...
  case HAIKU_EVENT_POINTER_MOVE:
    ...
    server->real_mouse_in_toplevel_id = message->pointer.mouse_window_id;
    break;
  case HAIKU_EVENT_BUTTON_PRESS:
  case HAIKU_EVENT_BUTTON_RELEASE:
    if (message->base.type == HAIKU_EVENT_BUTTON_PRESS &&
        server->focused_window_id != message->pointer.mouse_window_id &&
        server->pointer_grab_window_id == -1)
      {
        haiku_server_window_raise (server, message->pointer.mouse_window_id);
        haiku_server_focus_window (server, message->pointer.mouse_window_id);
        haiku_server_flush (server);
      }
  ...
  case HAIKU_EVENT_TOUCH:
    if (message->touch.touch_type == 0 && message->touch.is_emulated &&
      ...
      {
        haiku_server_window_raise (server, message->touch.event_window_id);
        haiku_server_focus_window (server, message->touch.event_window_id);
        haiku_server_flush (server);
      }
    ...
    break;
  case HAIKU_EVENT_KEY_PRESS:
  case HAIKU_EVENT_KEY_RELEASE:
    server->last_state = message->key.state;
    break;
  case HAIKU_EVENT_GRAB_NOTIFY:
  case HAIKU_EVENT_UNGRAB_NOTIFY:
    break;
  case HAIKU_EVENT_CONFIGURE_NOTIFY:
	...
	{
	  haiku_server_window_raise (server, message->configure_notify.id);
	  haiku_server_focus_window (server, message->configure_notify.id);
	  haiku_server_flush (server);
	}
      }
    break;
  case HAIKU_EVENT_DELETE_NOTIFY:
    break;
  case HAIKU_EVENT_SCREEN_SIZE_CHANGED:
...
gboolean
haiku_server_lookahead_event (HaikuServer  *server,
				 const char         *types)
{
  HaikuInputMsg *message;
  GList *l;
...
static gboolean
is_pointer_event (HaikuInputMsg *message)
{
  return
    message->base.type == HAIKU_EVENT_ENTER ||
    message->base.type == HAIKU_EVENT_LEAVE ||
    message->base.type == HAIKU_EVENT_POINTER_MOVE ||
    message->base.type == HAIKU_EVENT_BUTTON_PRESS ||
    message->base.type == HAIKU_EVENT_BUTTON_RELEASE ||
    message->base.type == HAIKU_EVENT_SCROLL ||
    message->base.type == HAIKU_EVENT_GRAB_NOTIFY ||
    message->base.type == HAIKU_EVENT_UNGRAB_NOTIFY;
}

static void
process_input_message (HaikuServer *server,
		       HaikuInputMsg *message)
{
  gint32 client;
  ...
  haiku_events_got_input (message, client);
}

static void
process_input_messages (HaikuServer *server)
{
  HaikuInputMsg *message;
...
static void
fake_configure_notify (HaikuServer *server,
		       HaikuWindow *window)
{
  HaikuInputMsg ev = { {0} };

  ev.base.type = HAIKU_EVENT_CONFIGURE_NOTIFY;
...
static guint32 *
parse_pointer_data (guint32 *p, HaikuInputPointerMsg *data)
{
...
static guint32 *
parse_touch_data (guint32 *p, HaikuInputTouchMsg *data)
{
...
static void
update_future_pointer_info (HaikuServer *server, HaikuInputPointerMsg *data)
{
...
static void
parse_input_message (HaikuInput *input, const unsigned char *message)
{
  HaikuServer *server = input->server;
  HaikuInputMsg msg;
  guint32 *p;
  gint64 time_;
  ...
  case HAIKU_EVENT_POINTER_MOVE: /* Mouse move */
    p = parse_pointer_data (p, &msg.pointer);
    update_future_pointer_info (server, &msg.pointer);
    break;
  ...
  while (input->buffer->len > 2)
    {
      ...
      switch (code) {
      case HAIKU_WS_CNX_CLOSE:
        break; /* hang around anyway */
      case HAIKU_WS_BINARY:
        if (!fin)
          {
#ifdef DEBUG_WEBSOCKETS
        ...
        break;
      case HAIKU_WS_CNX_PING:
        haiku_output_pong (input->output);
        break;
      case HAIKU_WS_CNX_PONG:
        break; /* we never send pings, but tolerate pongs */
      case HAIKU_WS_TEXT:
      case HAIKU_WS_CONTINUATION:
      default:
        {
...
static gboolean
process_input_idle_cb (HaikuServer *server)
{
...
static void
queue_process_input_at_idle (HaikuServer *server)
{
...
static gboolean
haiku_server_read_all_input_nonblocking (HaikuInput *input)
{
  GInputStream *in;
  gssize res;
  guint8 buffer[1024];
...
static void
haiku_server_consume_all_input (HaikuServer *server)
{
  haiku_server_read_all_input_nonblocking (server->input);
	       ...
	       HaikuInput *input)
{
  ...
  if (!haiku_server_read_all_input_nonblocking (input))
    return FALSE;
...
guint32
haiku_server_get_next_serial (HaikuServer *server)
{
  if (server->output)
    return haiku_output_get_next_serial (server->output);

  return server->saved_serial;
}

void
haiku_server_get_screen_size (HaikuServer   *server,
				 guint32          *width,
				 guint32          *height)
{
...
void
haiku_server_flush (HaikuServer *server)
{
  if (server->output &&
      !haiku_output_flush (server->output))
    {
      server->saved_serial = haiku_output_get_next_serial (server->output);
      haiku_output_free (server->output);
      server->output = NULL;
    }
}
...
static HaikuInputMsg *
haiku_server_block_for_input (HaikuServer *server, char op,
...
start_input (HttpRequest *request)
{
  ...
  input->output =
    haiku_output_new (g_io_stream_get_output_stream (request->connection), 0);
...
static void
start (HaikuInput *input)
{
  HaikuServer *server;

  input->active = TRUE;

  server = HAIKU_SERVER (input->server);

  if (server->output)
    {
      haiku_output_disconnected (server->output);
      haiku_output_flush (server->output);
    }

  if (server->input != NULL)
    {
      haiku_input_free (server->input);
      server->input = NULL;
    }
  ...
  haiku_output_set_next_serial (server->output, server->saved_serial);
  haiku_output_flush (server->output);

  haiku_server_resync_windows (server);

  if (server->pointer_grab_window_id != -1)
    haiku_output_grab_pointer (server->output,
...
got_request (HttpRequest *request)
{
  ...
  else if (strcmp (escaped, "/haiku.js") == 0)
    send_data (request, "text/javascript", haiku_js, G_N_ELEMENTS(haiku_js) - 1);
			    ...
			    GObject           *source_object)
{
  ...
  request->server = HAIKU_SERVER (source_object);
                     ...
                     GError     **error)
{
  ...
  server = g_object_new (HAIKU_TYPE_SERVER, NULL);
  server->port = port;
...
HaikuServer *
haiku_server_on_unix_socket_new (char *address, GError **error)
{
  HaikuServer *server;
...
guint32
haiku_server_get_last_seen_time (HaikuServer *server)
{
  haiku_server_consume_all_input (server);
  return (guint32) server->last_seen_time;
}

void
haiku_server_query_mouse (HaikuServer *server,
			     guint32            *toplevel,
			     gint32             *root_x,
			     gint32             *root_y,
			     guint32            *mask)
...
void
haiku_server_destroy_window (HaikuServer *server,
				gint id)
{
  HaikuWindow *window;
  ...
  if (server->output)
    haiku_output_destroy_surface (server->output,
				     id);
  ...
  if (transient_for != -1)
    {
      ...
      if (window != NULL)
        {
	  haiku_server_focus_window (server, transient_for);
	  haiku_server_flush (server);
	}
    }
}
...
gboolean
haiku_server_window_show (HaikuServer *server,
			     gint id)
{
  HaikuWindow *window;
  gboolean sent = FALSE;
  ...
  if (server->output)
    {
      haiku_output_show_surface (server->output, window->id);
      sent = TRUE;
    }
...
gboolean
haiku_server_window_hide (HaikuServer *server,
			     gint id)
{
  HaikuWindow *window;
  gboolean sent = FALSE;
  ...
  if (server->output)
    {
      haiku_output_hide_surface (server->output, window->id);
      sent = TRUE;
    }
  return sent;
}
...
void
haiku_server_window_raise (HaikuServer *server,
                              gint id)
{
  HaikuWindow *window;
  ...
  if (server->output)
    haiku_output_raise_surface (server->output, window->id);
}

void
haiku_server_set_show_keyboard (HaikuServer *server,
                                   gboolean show)
{
  ...
  if (server->output)
    {
      haiku_output_set_show_keyboard (server->output, server->show_keyboard);
      haiku_server_flush (server);
   }
}

void
haiku_server_window_lower (HaikuServer *server,
                              gint id)
{
  HaikuWindow *window;
  ...
  if (server->output)
    haiku_output_lower_surface (server->output, window->id);
}

void
haiku_server_window_set_transient_for (HaikuServer *server,
					  gint id, gint parent)
{
  HaikuWindow *window;
  ...
  if (server->output)
    {
      haiku_output_set_transient_for (server->output, window->id, window->transient_for);
      haiku_server_flush (server);
    }
}

void
haiku_server_window_set_modal_hint (HaikuServer *server,
...
gboolean
haiku_server_has_client (HaikuServer *server)
{
  return server->output != NULL;
}

void
haiku_server_window_update (HaikuServer *server,
			       gint id,
			       cairo_surface_t *surface)
{
  ...
  buffer = haiku_buffer_create (window->width, window->height,
  ...
  if (server->output != NULL)
    {
      ...
      haiku_output_put_buffer (server->output, window->id,
                                  window->buffer, buffer);
    }

  if (window->buffer)
    haiku_buffer_destroy (window->buffer);

  window->buffer = buffer;
}

gboolean
haiku_server_window_move_resize (HaikuServer *server,
				    gint id,
				    gboolean with_move,
				    int x,
				    int y,
				    ...
				    int height)
{
  ...
  if (server->output != NULL)
    {
      haiku_output_move_resize_surface (server->output,
					   window->id,
					   with_move, x, y,
...
void
haiku_server_focus_window (HaikuServer *server,
                              gint new_focused_window)
{
  HaikuInputMsg focus_msg;

  if (server->focused_window_id == new_focused_window)
    return;

  memset (&focus_msg, 0, sizeof (focus_msg));
  focus_msg.base.type = HAIKU_EVENT_FOCUS;
  focus_msg.base.time = haiku_server_get_last_seen_time (server);
  focus_msg.focus.old_id = server->focused_window_id;
  focus_msg.focus.new_id = new_focused_window;

  haiku_events_got_input (&focus_msg, -1);
...
guint32
haiku_server_grab_pointer (HaikuServer *server,
			      gint client_id,
			      gint id,
...
guint32
haiku_server_ungrab_pointer (HaikuServer *server,
				guint32    time_)
{
  guint32 serial;
  ...
  if (server->output)
    {
      serial = haiku_output_ungrab_pointer (server->output);
      haiku_server_flush (server);
    }
  else
    {
...
cairo_surface_t *
haiku_server_open_surface (HaikuServer *server,
			      guint32 id,
			      char *name,
			      int width,
			      int height)
...
guint32
haiku_server_new_window (HaikuServer *server,
			    int x,
			    int y,
			    int width,
			    int height,
			    gboolean is_temp)
{
  ...
  if (server->output)
    haiku_output_new_surface (server->output,
				 window->id,
				 window->x,
				 window->y,
				 window->width,
...
static void
haiku_server_resync_windows (HaikuServer *server)
{
  GList *l;
  ...
  if (server->show_keyboard)
    haiku_output_set_show_keyboard (server->output, TRUE);
