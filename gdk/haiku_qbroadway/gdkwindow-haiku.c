...
static void     gdk_window_haiku_set_background     (GdkWindow      *window,
                                                   cairo_pattern_t *pattern);

static void        gdk_window_impl_haiku_finalize   (GObject            *object);

static const cairo_user_data_key_t gdk_haiku_cairo_key;
...
G_DEFINE_TYPE (GdkWindowImplHaiku,
	       gdk_window_impl_haiku,
	       GDK_TYPE_WINDOW_IMPL)

static GdkDisplay *
find_haiku_display (void)
{
  GdkDisplay *display;
  GSList *list, *l;
  ...
  for (l = list; l; l = l->next)
    {
      if (GDK_IS_HAIKU_DISPLAY (l->data))
        {
          display = l->data;
          break; 
        }
...
update_dirty_windows_and_sync (void)
{
  ...
  display = GDK_HAIKU_DISPLAY (find_haiku_display ());
  g_assert (display != NULL);

  updated_surface = FALSE;
  for (l = display->toplevels; l != NULL; l = l->next)
    {
      GdkWindowImplHaiku *impl = l->data;

      if (impl->dirty)
	{
	  impl->dirty = FALSE;
	  updated_surface = TRUE;
	  _gdk_haiku_server_window_update (display->server,
					      impl->id,
					      impl->surface);
	}
    }
...
flush_idle (gpointer data)
{
  ...
  gdk_display_flush (find_haiku_display ());
...
static void
gdk_window_impl_haiku_init (GdkWindowImplHaiku *impl)
{
...
gdk_window_impl_haiku_finalize (GObject *object)
{
  ...
  GdkHaikuDisplay *haiku_display;

  g_return_if_fail (GDK_IS_WINDOW_IMPL_HAIKU (object));

  impl = GDK_WINDOW_IMPL_HAIKU (object);

  wrapper = impl->wrapper;

  _gdk_haiku_window_grab_check_destroy (wrapper);

  haiku_display = GDK_HAIKU_DISPLAY (gdk_window_get_display (impl->wrapper));

  g_hash_table_remove (haiku_display->id_ht, GINT_TO_POINTER(impl->id));
  ...
  haiku_display->toplevels = g_list_remove (haiku_display->toplevels, impl);

  G_OBJECT_CLASS (gdk_window_impl_haiku_parent_class)->finalize (object);
}

void
_gdk_haiku_screen_init_root_window (GdkScreen * screen)
{
  GdkWindow *window;
  GdkWindowImplHaiku *impl;
  GdkHaikuScreen *haiku_screen;

  haiku_screen = GDK_HAIKU_SCREEN (screen);

  g_assert (haiku_screen->root_window == NULL);

  haiku_screen->root_window = g_object_new (GDK_TYPE_HAIKU_WINDOW, NULL);

  window = haiku_screen->root_window;
  window->impl = g_object_new (GDK_TYPE_WINDOW_IMPL_HAIKU, NULL);
  window->impl_window = window;
  window->visual = gdk_screen_get_system_visual (screen);

  impl = GDK_WINDOW_IMPL_HAIKU (window->impl);
  ...
  _gdk_window_update_size (haiku_screen->root_window);
}
					  ...
					  gint           attributes_mask)
{
  ...
  haiku_display = GDK_HAIKU_DISPLAY (display);

  impl = g_object_new (GDK_TYPE_WINDOW_IMPL_HAIKU, NULL);
  window->impl = (GdkWindowImpl *)impl;
  impl->id = _gdk_haiku_server_new_window (haiku_display->server,
					      window->x,
					      window->y,
					      window->width,
					      window->height,
					      window->window_type == GDK_WINDOW_TEMP);
  g_hash_table_insert (haiku_display->id_ht, GINT_TO_POINTER(impl->id), window);
  impl->wrapper = window;
  ...
  haiku_display->toplevels = g_list_prepend (haiku_display->toplevels, impl);

  connect_frame_clock (window);
}

void
_gdk_haiku_window_resize_surface (GdkWindow *window)
{
  ...
  if (impl->surface)
    {
      ...
      impl->surface = _gdk_haiku_server_create_surface (gdk_window_get_width (impl->wrapper),
							   gdk_window_get_height (impl->wrapper));
    }

  if (impl->ref_surface)
    {
      cairo_surface_set_user_data (impl->ref_surface, &gdk_haiku_cairo_key,
				   NULL, NULL);
...
{
  GdkWindowImplHaiku *impl = data;

  impl->ref_surface = NULL;
}

static cairo_surface_t *
gdk_window_haiku_ref_cairo_surface (GdkWindow *window)
{
  GdkWindowImplHaiku *impl = GDK_WINDOW_IMPL_HAIKU (window->impl);
  int w, h;

  if (GDK_IS_WINDOW_IMPL_HAIKU (window) &&
  ...
  if (!impl->surface)
    impl->surface = _gdk_haiku_server_create_surface (w, h);
...
static void
_gdk_haiku_window_destroy (GdkWindow *window,
			      gboolean   recursing,
			      gboolean   foreign_destroy)
{
  ...
  _gdk_haiku_selection_window_destroyed (window);
  _gdk_haiku_window_grab_check_destroy (window);
  ...
  haiku_display = GDK_HAIKU_DISPLAY (gdk_window_get_display (window));
  g_hash_table_remove (haiku_display->id_ht, GINT_TO_POINTER(impl->id));

  _gdk_haiku_server_destroy_window (haiku_display->server,
				       impl->id);
}
...
static void
gdk_window_haiku_show (GdkWindow *window, gboolean already_mapped)
{
  GdkWindowImplHaiku *impl;
  GdkHaikuDisplay *haiku_display;
  ...
  haiku_display = GDK_HAIKU_DISPLAY (gdk_window_get_display (window));
  if (_gdk_haiku_server_window_show (haiku_display->server, impl->id))
    queue_flush (window);

}

static void
gdk_window_haiku_hide (GdkWindow *window)
{
  GdkWindowImplHaiku *impl;
  GdkHaikuDisplay *haiku_display;
  ...
  _gdk_haiku_window_grab_check_unmap (window,
					 _gdk_haiku_server_get_next_serial (haiku_display->server));

  if (_gdk_haiku_server_window_hide (haiku_display->server, impl->id))
    queue_flush (window);
				 ...
				 gint       height)
{
  ...
  if (width > 0 || height > 0)
	  ...
	  height != window->height)
	{
	  ...
	  _gdk_haiku_window_resize_surface (window);
	}
    }

  _gdk_haiku_server_window_move_resize (haiku_display->server,
					   impl->id,
					   with_move,
					   x, y,
			   ...
			   guint32    timestamp)
{
  ...
  impl = GDK_WINDOW_IMPL_HAIKU (window->impl);
  haiku_display = GDK_HAIKU_DISPLAY (gdk_window_get_display (window));
  _gdk_haiku_server_window_focus (haiku_display->server,
				     impl->id);
}
				    ...
				    gboolean   modal)
{
  ...
  display = GDK_HAIKU_DISPLAY (gdk_window_get_display (impl->wrapper));
  _gdk_haiku_server_window_set_modal_hint (display->server, impl->id, impl->modal_hint);
}
				       ...
				       GdkWindow *parent)
{
  ...
  parent_id = 0;
  if (parent)
    parent_id = GDK_WINDOW_IMPL_HAIKU (parent->impl)->id;

  impl->transient_for = parent_id;

  display = GDK_HAIKU_DISPLAY (gdk_window_get_display (impl->wrapper));
  _gdk_haiku_server_window_set_transient_for (display->server, impl->id, impl->transient_for);
}
				       ...
				       GdkCursor *cursor)
{
  ...
  if (!cursor)
    ...
    {
      _gdk_haiku_cursor_update_theme (cursor);
...
static void
gdk_haiku_window_end_paint (GdkWindow *window)
{
  GdkWindowImplHaiku *impl;
  impl = GDK_WINDOW_IMPL_HAIKU (window->impl);
  impl->dirty = TRUE;
...
struct _MoveResizeData
{
  ...
  HaikuInputMsg *moveresize_pending_event;
};
		      ...
		      gboolean    create)
{
  ...
  mv_resize = haiku_display->move_resize_data;
  ...
  if (!mv_resize && create)
    {
      ...
      haiku_display->move_resize_data = mv_resize;
    }
		      ...
		      HaikuInputMsg *event)
{
  ...
  return !_gdk_haiku_server_lookahead_event (haiku_display->server, "mb");
}

gboolean
_gdk_haiku_moveresize_handle_event (GdkDisplay *display,
				       HaikuInputMsg *event)
{
  guint button_mask = 0;
  ...
  switch (event->base.type)
              ...
              if (mv_resize->moveresize_pending_event)
                *mv_resize->moveresize_pending_event = *event;
                ...
                mv_resize->moveresize_pending_event =
                  g_memdup (event, sizeof (HaikuInputMsg));
    ...
    case HAIKU_EVENT_BUTTON_RELEASE:
      update_pos (mv_resize,
		  event->pointer.root_x,
		  event->pointer.root_y);
...
gboolean
_gdk_haiku_moveresize_configure_done (GdkDisplay *display,
					 GdkWindow  *window)
{
  HaikuInputMsg *tmp_event;
  MoveResizeData *mv_resize = get_move_resize_data (display, FALSE);
  ...
  if (mv_resize->moveresize_pending_event)
    {
      ...
      _gdk_haiku_moveresize_handle_event (display, tmp_event);
      g_free (tmp_event);
    }
...
static gboolean
gdk_haiku_window_beep (GdkWindow *window)
{
  return FALSE;
}
...
guint32
gdk_haiku_get_last_seen_time (GdkWindow  *window)
{
  GdkDisplay *display;

  display = gdk_window_get_display (window);
  return _gdk_haiku_server_get_last_seen_time (GDK_HAIKU_DISPLAY (display)->server);
}

static void
gdk_window_impl_haiku_class_init (GdkWindowImplHaikuClass *klass)
{
  ...
  impl_class->ref_cairo_surface = gdk_window_haiku_ref_cairo_surface;
  impl_class->show = gdk_window_haiku_show;
  impl_class->hide = gdk_window_haiku_hide;
  ...
  impl_class->destroy = _gdk_haiku_window_destroy;
  ...
  impl_class->end_paint = gdk_haiku_window_end_paint;
  impl_class->beep = gdk_haiku_window_beep;
