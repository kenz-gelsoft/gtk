...
static gboolean
verify_block_match (HaikuBuffer *buffer, int x, int y,
                    HaikuBuffer *prev, struct entry *entry)
{
  int i;
  void *old, *match;
  int w1, w2, h1, h2;
...
static void
insert_block (HaikuBuffer *buffer, guint32 h, int x, int y)
{
  struct entry *entry;
  int i;
  guint32 collision = 0;
...
static struct entry *
lookup_block (HaikuBuffer *prev, guint32 h)
{
  guint32 i;
  struct entry *entry;
...
void
haiku_buffer_destroy (HaikuBuffer *buffer)
{
  g_free (buffer->data);
  g_free (buffer->table);
  g_free (buffer);
...
int
haiku_buffer_get_width (HaikuBuffer *buffer)
{
  return buffer->width;
}

int
haiku_buffer_get_height (HaikuBuffer *buffer)
{
...
HaikuBuffer *
haiku_buffer_create (int width, int height, guint8 *data, int stride)
{
  HaikuBuffer *buffer;
  int y, bits_required;

  buffer = g_new0 (HaikuBuffer, 1);
  buffer->width = width;
...
void
haiku_buffer_encode (HaikuBuffer *buffer, HaikuBuffer *prev, GString *dest)
{
  struct entry *entry;
  int i, j, k;
  int x0, x1, y0, y1;
