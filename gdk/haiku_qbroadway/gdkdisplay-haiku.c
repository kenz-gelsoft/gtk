...
static void   gdk_haiku_display_dispose            (GObject            *object);
static void   gdk_haiku_display_finalize           (GObject            *object);
...
gdk_haiku_display_init (GdkHaikuDisplay *display)
{
  ...
  display->monitor = g_object_new (GDK_TYPE_HAIKU_MONITOR,
                                   "display", display,
                                   NULL);
...
gdk_event_init (GdkDisplay *display)
{
  ...
  haiku_display->event_source = _gdk_haiku_event_source_new (display);
}
...
_gdk_haiku_display_open (const gchar *display_name)
{
  ...
  haiku_display->screens = g_new (GdkScreen *, 1);
  haiku_display->screens[0] = _gdk_haiku_screen_new (display, 0);

  /* We need to initialize events after we have the screen
   * structures in places
   */
  _gdk_haiku_screen_events_init (haiku_display->screens[0]);

  /*set the default screen */
  haiku_display->default_screen = haiku_display->screens[0];

  display->device_manager = _gdk_haiku_device_manager_new (display);

  gdk_event_init (display);

  _gdk_haiku_display_init_dnd (display);

  _gdk_haiku_screen_setup (haiku_display->screens[0]);

  if (display_name == NULL)
    display_name = g_getenv ("HAIKU_DISPLAY");

  haiku_display->server = _gdk_haiku_server_new (display_name, &error);
  if (haiku_display->server == NULL)
    {
      GDK_NOTE (MISC, g_message ("Unable to init Haiku server: %s\n", error->message));
      g_error_free (error);
      return NULL;
    }
...
gdk_haiku_display_get_default_screen (GdkDisplay *display)
{
  ...
  return GDK_HAIKU_DISPLAY (display)->default_screen;
}
...
gdk_haiku_display_sync (GdkDisplay *display)
{
  ...
  g_return_if_fail (GDK_IS_HAIKU_DISPLAY (display));

  _gdk_haiku_server_sync (haiku_display->server);
}
...
gdk_haiku_display_flush (GdkDisplay *display)
{
  ...
  _gdk_haiku_server_flush (haiku_display->server);
}
...
gdk_haiku_display_dispose (GObject *object)
{
  ...
  _gdk_screen_close (haiku_display->screens[0]);

  if (haiku_display->event_source)
    {
      g_source_destroy (haiku_display->event_source);
      g_source_unref (haiku_display->event_source);
      haiku_display->event_source = NULL;
    }
...
gdk_haiku_display_finalize (GObject *object)
{
  ...
  /* Keymap */
  if (haiku_display->keymap)
    g_object_unref (haiku_display->keymap);

  _gdk_haiku_cursor_display_finalize (GDK_DISPLAY_OBJECT(haiku_display));

  /* Free all GdkScreens */
  g_object_unref (haiku_display->screens[0]);
  g_free (haiku_display->screens);

  g_object_unref (haiku_display->monitor);
...
gdk_haiku_display_get_next_serial (GdkDisplay *display)
{
  ...
  return _gdk_haiku_server_get_next_serial (haiku_display->server);
}

void
gdk_haiku_display_show_keyboard (GdkHaikuDisplay *display)
{
  g_return_if_fail (GDK_IS_HAIKU_DISPLAY (display));

  _gdk_haiku_server_set_show_keyboard (display->server, TRUE);
}

void
gdk_haiku_display_hide_keyboard (GdkHaikuDisplay *display)
{
  g_return_if_fail (GDK_IS_HAIKU_DISPLAY (display));

  _gdk_haiku_server_set_show_keyboard (display->server, FALSE);
}
