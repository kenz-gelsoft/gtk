...
void
_gdk_haiku_events_got_input (HaikuInputMsg *message)
{
  GdkDisplay *display;
  ...
  for (d = list; d; d = d->next)
    {
      if (GDK_IS_HAIKU_DISPLAY (d->data))
        {
          display = d->data;
          break;
        }
  ...
  device_manager = GDK_HAIKU_DEVICE_MANAGER (gdk_display_get_device_manager (display));
  G_GNUC_END_IGNORE_DEPRECATIONS;

  switch (message->base.type) {
  case HAIKU_EVENT_ENTER:
    window = g_hash_table_lookup (display_haiku->id_ht, GINT_TO_POINTER (message->pointer.event_window_id));
    if (window)
      {
  ...
  case HAIKU_EVENT_POINTER_MOVE:
    if (_gdk_haiku_moveresize_handle_event (display, message))
      break;
  ...
  case HAIKU_EVENT_BUTTON_RELEASE:
	...
	_gdk_haiku_moveresize_handle_event (display, message))
      break;
  ...
  case HAIKU_EVENT_TOUCH:
    window = g_hash_table_lookup (display_haiku->id_ht, GINT_TO_POINTER (message->touch.event_window_id));
    if (window)
      {
        ...
        case 2:
          ...
          break;
        default:
          g_printerr ("_gdk_haiku_events_got_input - Unknown touch type %d\n", message->touch.touch_type);
        }

        if (event_type != GDK_TOUCH_BEGIN &&
            message->touch.is_emulated && _gdk_haiku_moveresize_handle_event (display, message))
          break;
  ...
  case HAIKU_EVENT_KEY_RELEASE:
    window = g_hash_table_lookup (display_haiku->id_ht,
  ...
  case HAIKU_EVENT_CONFIGURE_NOTIFY:
    window = g_hash_table_lookup (display_haiku->id_ht, GINT_TO_POINTER (message->configure_notify.id));
    if (window)
      {
	...
	if (window->resize_count >= 1)
	  {
	    ...
	    if (window->resize_count == 0)
	      _gdk_haiku_moveresize_configure_done (display, window);
	  }
      }
    break;

  case HAIKU_EVENT_DELETE_NOTIFY:
    window = g_hash_table_lookup (display_haiku->id_ht, GINT_TO_POINTER (message->delete_notify.id));
    if (window)
      {
  ...
  case HAIKU_EVENT_SCREEN_SIZE_CHANGED:
    ...
    _gdk_haiku_screen_size_changed (screen, &message->screen_resize_notify);
    break;

  case HAIKU_EVENT_FOCUS:
    window = g_hash_table_lookup (display_haiku->id_ht, GINT_TO_POINTER (message->focus.old_id));
    if (window)
      {
    ...
    window = g_hash_table_lookup (display_haiku->id_ht, GINT_TO_POINTER (message->focus.new_id));
    if (window)
      {
	event = gdk_event_new (GDK_FOCUS_CHANGE);
	event->focus_change.window = g_object_ref (window);
  ...
  default:
    g_printerr ("_gdk_haiku_events_got_input - Unknown input command %c\n", message->base.type);
    break;
  }
}
...
GSource *
_gdk_haiku_event_source_new (GdkDisplay *display)
{
  GSource *source;
  ...
  name = g_strdup_printf ("GDK Haiku Event source (%s)",
