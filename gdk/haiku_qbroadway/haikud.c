...
/* Serials:
 *
 * Haiku tracks serials for all clients primarily to get the right behaviour wrt
...
static void
client_free (HaikuClient *client)
{
...
static void
client_disconnected (HaikuClient *client)
{
  GList *l;
  ...
  for (l = client->windows; l != NULL; l = l->next)
    haiku_server_destroy_window (server,
...
static gboolean
disconnect_idle_cb (HaikuClient *client)
{
  client->disconnect_idle = 0;
...
static void
client_disconnect_in_idle (HaikuClient *client)
{
  if (client->disconnect_idle == 0)
...
static void
add_client_serial_mapping (HaikuClient *client,
			   guint32 client_serial,
			   guint32 daemon_serial)
{
  HaikuSerialMapping *map;
  GSList *last;
  ...
  map = g_new0 (HaikuSerialMapping, 1);
  map->client_serial = client_serial;
...
static guint32
get_client_serial (HaikuClient *client, guint32 daemon_serial)
{
  HaikuSerialMapping *map;
  GSList *l, *found;
...
static void
client_handle_request (HaikuClient *client,
		       HaikuRequest *request)
{
  HaikuReplyNewWindow reply_new_window;
  HaikuReplySync reply_sync;
  HaikuReplyQueryMouse reply_query_mouse;
  HaikuReplyGrabPointer reply_grab_pointer;
  HaikuReplyUngrabPointer reply_ungrab_pointer;
  cairo_surface_t *surface;
  guint32 before_serial, now_serial;

  before_serial = haiku_server_get_next_serial (server);

  switch (request->base.type)
    {
    case HAIKU_REQUEST_NEW_WINDOW:
      reply_new_window.id =
	haiku_server_new_window (server,
	...
	g_list_prepend (client->windows,
      ...
      send_reply (client, request, (HaikuReply *)&reply_new_window, sizeof (reply_new_window),
		  HAIKU_REPLY_NEW_WINDOW);
      break;
    case HAIKU_REQUEST_FLUSH:
      haiku_server_flush (server);
      break;
    case HAIKU_REQUEST_SYNC:
      haiku_server_flush (server);
      send_reply (client, request, (HaikuReply *)&reply_sync, sizeof (reply_sync),
		  HAIKU_REPLY_SYNC);
      break;
    case HAIKU_REQUEST_QUERY_MOUSE:
      haiku_server_query_mouse (server,
				   &reply_query_mouse.toplevel,
				   &reply_query_mouse.root_x,
				   &reply_query_mouse.root_y,
				   &reply_query_mouse.mask);
      send_reply (client, request, (HaikuReply *)&reply_query_mouse, sizeof (reply_query_mouse),
		  HAIKU_REPLY_QUERY_MOUSE);
      break;
    case HAIKU_REQUEST_DESTROY_WINDOW:
      client->windows =
	g_list_remove (client->windows,
		       GUINT_TO_POINTER (request->destroy_window.id));
      haiku_server_destroy_window (server, request->destroy_window.id);
      break;
    case HAIKU_REQUEST_SHOW_WINDOW:
      haiku_server_window_show (server, request->show_window.id);
      break;
    case HAIKU_REQUEST_HIDE_WINDOW:
      haiku_server_window_hide (server, request->hide_window.id);
      break;
    case HAIKU_REQUEST_SET_TRANSIENT_FOR:
      haiku_server_window_set_transient_for (server,
						request->set_transient_for.id,
						request->set_transient_for.parent);
      break;
    case HAIKU_REQUEST_UPDATE:
      surface = haiku_server_open_surface (server,
					      request->update.id,
					      request->update.name,
      ...
      if (surface != NULL)
	{
	  haiku_server_window_update (server,
					 request->update.id,
					 surface);
	  cairo_surface_destroy (surface);
	}
      break;
    case HAIKU_REQUEST_MOVE_RESIZE:
      haiku_server_window_move_resize (server,
      ...
      break;
    case HAIKU_REQUEST_GRAB_POINTER:
      reply_grab_pointer.status =
	haiku_server_grab_pointer (server,
				      client->id,
				      request->grab_pointer.id,
				      request->grab_pointer.owner_events,
				      request->grab_pointer.event_mask,
				      request->grab_pointer.time_);
      send_reply (client, request, (HaikuReply *)&reply_grab_pointer, sizeof (reply_grab_pointer),
		  HAIKU_REPLY_GRAB_POINTER);
      break;
    case HAIKU_REQUEST_UNGRAB_POINTER:
      reply_ungrab_pointer.status =
	haiku_server_ungrab_pointer (server,
					request->ungrab_pointer.time_);
      send_reply (client, request, (HaikuReply *)&reply_ungrab_pointer, sizeof (reply_ungrab_pointer),
		  HAIKU_REPLY_UNGRAB_POINTER);
      break;
    case HAIKU_REQUEST_FOCUS_WINDOW:
      haiku_server_focus_window (server, request->focus_window.id);
      break;
    case HAIKU_REQUEST_SET_SHOW_KEYBOARD:
      haiku_server_set_show_keyboard (server, request->set_show_keyboard.show_keyboard);
      break;
    case HAIKU_REQUEST_SET_MODAL_HINT:
      haiku_server_window_set_modal_hint (server,
  ...
  now_serial = haiku_server_get_next_serial (server);
		...
		gpointer user_data)
{
  ...
  if (res > 0)
      ...
      while (remaining >= sizeof (guint32))
	{
	  ...
	  if (size <= remaining)
	    {
	      client_handle_request (client, (HaikuRequest *)buffer);
		 ...
		 GObject           *source_object)
{
  ...
  ev.base.type = HAIKU_EVENT_SCREEN_SIZE_CHANGED;
  ev.base.serial = haiku_server_get_next_serial (server) - 1;
  ev.base.time = haiku_server_get_last_seen_time (server);
  haiku_server_get_screen_size (server,
				   &ev.screen_resize_notify.width,
				   &ev.screen_resize_notify.height);

  haiku_events_got_input (&ev,
			     client->id);
...
main (int argc, char *argv[])
{
  ...
  context = g_option_context_new ("[:DISPLAY] - haiku display daemon");
  ...
  if (argc > 1)
    {
      if (*argv[1] != ':')
	{
	  g_printerr ("Usage haikud [:DISPLAY]\n");
	  exit (1);
	}
      display = argv[1];
    }
  ...
  if (g_str_has_prefix (display, ":tcp"))
  ...
  else if (display[0] == ':' && g_ascii_isdigit(display[1]))
    {
      ...
      basename = g_strdup_printf ("haiku%d.socket", port + 1);
  ...
  if (unixsocket_address != NULL)
    server = haiku_server_on_unix_socket_new (unixsocket_address, &error);
  else
    server = haiku_server_new (http_address,
                                  http_port,
                                  ssl_cert,
                                  ssl_key,
                                  &error);
...
get_event_size (int type)
{
  switch (type)
    ...
    case HAIKU_EVENT_LEAVE:
      return sizeof (HaikuInputCrossingMsg);
    case HAIKU_EVENT_POINTER_MOVE:
      return sizeof (HaikuInputPointerMsg);
    case HAIKU_EVENT_BUTTON_PRESS:
    case HAIKU_EVENT_BUTTON_RELEASE:
      return sizeof (HaikuInputButtonMsg);
    case HAIKU_EVENT_SCROLL:
      return sizeof (HaikuInputScrollMsg);
    case HAIKU_EVENT_TOUCH:
      return sizeof (HaikuInputTouchMsg);
    case HAIKU_EVENT_KEY_PRESS:
    case HAIKU_EVENT_KEY_RELEASE:
      return  sizeof (HaikuInputKeyMsg);
    case HAIKU_EVENT_GRAB_NOTIFY:
    case HAIKU_EVENT_UNGRAB_NOTIFY:
      return sizeof (HaikuInputGrabReply);
    case HAIKU_EVENT_CONFIGURE_NOTIFY:
      return  sizeof (HaikuInputConfigureNotify);
    case HAIKU_EVENT_DELETE_NOTIFY:
      return sizeof (HaikuInputDeleteNotify);
    case HAIKU_EVENT_SCREEN_SIZE_CHANGED:
      return sizeof (HaikuInputScreenResizeNotify);
    case HAIKU_EVENT_FOCUS:
      return sizeof (HaikuInputFocusMsg);
    default:
...
void
haiku_events_got_input (HaikuInputMsg *message,
			   gint32 client_id)
{
  GList *l;
  HaikuReplyEvent reply_event;
  gsize size;
  guint32 daemon_serial;

  size = get_event_size (message->base.type);
  g_assert (sizeof (HaikuReplyBase) + size <= sizeof (HaikuReplyEvent));

  memset (&reply_event, 0, sizeof (HaikuReplyEvent));
  ...
  for (l = clients; l != NULL; l = l->next)
	  ...
	  client->id == client_id)
	{
	  ...
	  send_reply (client, NULL, (HaikuReply *)&reply_event,
		      G_STRUCT_OFFSET (HaikuReplyEvent, msg) + size,
		      HAIKU_REPLY_EVENT);
	}
    }
}
